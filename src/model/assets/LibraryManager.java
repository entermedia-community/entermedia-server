...
public class LibraryManager extends EnterMediaObject {
	// If a user picks a library and collection then we just need to add the
	// category

	// If we are scanning a hot folder then we add categories anyways. So this is
	// not needed
	...
	public void assignLibraries(MediaArchive mediaarchive, Collection assets) {
		...
		for (Iterator iterator = assets.iterator(); iterator.hasNext();) {
			...
			for (Iterator iterator2 = asset.getCategories().iterator(); iterator2.hasNext();) {
				...
				Data collection = librarycollectionSearcher.query().orgroup("rootcategory", child.getParentCategories())
						.searchOne();
				if (collection == null) {
					HitTracker creates = librarycollectioncreationSearcher.query()
							.orgroup("category", child.getParentCategories()).search();
					for (Object hit : creates) {
						...
						if (collcat == null) {
							...
							librarycollectionSearcher.saveData(collection); // TODO: Lock root index category so that
																			// duplicate collections are not made
						}
					}
				}
