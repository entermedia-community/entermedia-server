...
public abstract class BasePushManager implements PushManager {
	...
	public void acceptPush(WebPageRequest inReq, MediaArchive archive) {
		...
		if (uploadFiles != null) {
			...
			while (iter.hasNext()) {
				...
				if (inputName.startsWith("original")) {
					...
					properties.saveFileAs(fileItem,
							"/WEB-INF/data/" + archive.getCatalogId() + "/assets/" + sourcepath + "/fulltext.txt",
							inReq.getUser());
				} else {
		...
		log.info("Received a pushed asset: " + target.getId() + " at " + target.getSourcePath() + " : "
				+ "details(k4 / edit status) : " + k4processed + " / " + editstatus);
	 ...
	 * }
	 * 
	 * Gets the approved assets (that are not marked for deletion) from remote
	 * server
	 * 
	 * @param inArchive
	 * 
	 * @return
	 * protected HashMap<String,Properties> getApprovedAssets(MediaArchive
	 * inArchive) {
	 * log.info("getApprovedAssets starting");
	 * String server = inArchive.getCatalogSettingValue("push_server_url");
	 * String remotecatalogid =
	 * inArchive.getCatalogSettingValue("push_target_catalogid");
	 * log.
	 * info("push_server_url = $server, push_target_catalogid = $remotecatalogid");
	 ...
	 * }
	 * 
	 * protected void processApprovedAssets(MediaArchive inArchive,
	 * Map<String,Properties> inMap){
	 * String catalogid = inArchive.getCatalogId();
	 * String server = inArchive.getCatalogSettingValue("push_server_url");
	 * String remotecatalogid =
	 * inArchive.getCatalogSettingValue("push_target_catalogid");
	 * String exportpath =
	 * inArchive.getCatalogSettingValue("push_download_exportpath");
	 * if (exportpath == null){
	 ...
	 * log.
	 * info("unable to process $key, name ($name) or url ($url) are null, skipping"
	 * );
	 * continue;
	 * }
	 ...
	 * }
	 * 
	 * protected Page getDownloadedAsset(MediaArchive inArchive, String inUrl,
	 * String inName, String inExportPath){
	 * String server = inArchive.getCatalogSettingValue("push_server_url");
	 ...
	 * }
	 * 
	 * protected Properties getAssetMetadata(MediaArchive inArchive, String
	 * inAssetId){
	 ...
	 * while(itr.hasNext()){
	 * Element e = itr.next();
	 * if (e.getName()==null || !e.getName().equals("property") ||
	 * e.attribute("id")==null || e.attribute("id").getValue().isEmpty()){
	 * // log.info("skipping ${inAssetId}: ${e}");
	 * continue;
	 * }
	 * String id = e.attribute("id").getValue();
	 * String valueid = e.attribute("valueid")!=null ?
	 * e.attribute("valueid").getValue() : null;
	 ...
	 * }
	 * 
	 * protected Asset createAsset(MediaArchive inArchive, Page inPage, Properties
	 * inMetadata){
	 * AssetImporter importer = (AssetImporter)
	 * inArchive.getModuleManager().getBean("assetImporter");
	 * String catalogid = inArchive.getCatalogId();
	 ...
	 * }
	 * Asset asset = (Asset)
	 * importer.createAssetFromExistingFile(inArchive,null,false,path);
	 * if (asset == null){
	 ...
	 * }
	 * importer.saveAsset(inArchive, null, asset); //TODO: check if zip
	 * functionality works
	 * return asset;
	 * }
	 * 
	 * protected Page moveDownloadedAsset(MediaArchive inArchive, Page inPage,
	 * Properties inMetadata){
	 * PropertyDetails props = inArchive.getAssetSearcher().getPropertyDetails();
	 * StringBuilder buf = new StringBuilder();
	 * //parser pattern specified in download sourcepath; look for keys in metadata
	 * and match fields in asset property definition
	 * //to determin datatype; otherwise just use exact string provided
	 * String pattern =
	 * inArchive.getCatalogSettingValue("push_download_sourcepath");
	 ...
	 * int start = -1;
	 * int end = -1;
	 * if ( (start = field.indexOf("{"))!=-1 && (end = field.indexOf("}"))!=-1 &&
	 * start < end){
	 ...
	 * if (prop.isDate()){
	 * isDate = true;
	 * } else if (param!=null){//check if it's formatted as a date because of
	 * provided param
	 * //if this succeeds then we know there's a difference in configs between
	 * client and server
	 ...
	 * }
	 * if (buf.toString().isEmpty()){
	 * String user = inMetadata.getProperty("owner","admin").trim();//make the
	 * default "admin" if owner has not been specified
	 ...
	 * }
	 * 
	 * protected void updateAssetEditStatus(MediaArchive inArchive, String
	 * inAssetId){
	 * String server = inArchive.getCatalogSettingValue("push_server_url");
	 * String remotecatalogid =
	 * inArchive.getCatalogSettingValue("push_target_catalogid");
