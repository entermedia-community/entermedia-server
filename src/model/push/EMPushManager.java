...
public class EMPushManager extends BasePushManager implements PushManager
{
	...
	public void uploadGenerated(MediaArchive archive, User inUser, Asset target, List savequeue)
	{
		...
		// archive.fireMediaEvent("importing/queueconversions", null, target); //This
		// will run right now, conflict?
		// archive.fireMediaEvent("conversions/runconversion", null, target); //This
		// will run right now, conflict?
	...
	public void pollRemotePublish(MediaArchive inArchive)
	{
		...
		// url = url +
		// "&field=remotempublishstatus&remotempublishstatus.value=new&operation=exact";
		...
		// nameValuePairs.add(new BasicNameValuePair("publishdestination.value",
		// ors.toString()));
	...
	public Map<String, String> upload(Asset inAsset, MediaArchive inArchive, String inUploadType, List<ContentItem> inFiles)
	{
		...
		String targetcatalogid = inArchive.getCatalogSettingValue("push_target_catalogid");
		// String password =
		// getUserManager().decryptPassword(getUserManager().getUser(account));
		...
		try
		{
			...
			for (Iterator iterator = inFiles.iterator(); iterator.hasNext();)
			{
				...
				// FilePart part = new FilePart(type + count, name, new File(
			...
			// {
			// StringBuffer buffer = new StringBuffer();
			// for (Iterator iterator = inAsset.getLibraries().iterator();
			// iterator.hasNext();)
			// {
			// String keyword = (String) iterator.next();
			// buffer.append( keyword );
			...
			//
			// MultipartRequestEntity entity = new MultipartRequestEntity(arrayOfparts,
			// method.getParams());
			// entity.
