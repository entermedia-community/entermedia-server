...
public class CategoryEditModule extends BaseMediaModule {
	...
	public void moveCategory(WebPageRequest inContext) throws OpenEditException {
		...
		if (category1 != null && category2 != null) {
			...
			if (category1 != category2 && category1.getParentCategory() != category2
	...
	public void resizeAllImages(WebPageRequest inContext) throws Exception {
		...
		try {
			List failures = editor.getMediaArchive().getCreatorManager().run(
					createthumb, createmedium, replacethumb, replacemedium,
					targetsearcher.search(q));
	...
	public void saveCategory(WebPageRequest inContext) throws OpenEditException {
		...
		if (currentCatalog != null && Boolean.parseBoolean(copy)) {
			...
			editor.getCurrentCategory().getParentCategory().addChild(
					currentCatalog);
	...
	public CategoryEditor getCategoryEditor(WebPageRequest inContext)
		...
		if (editor == null)
		{
			...
			inContext.putSessionValue("CategoryEditor" + mediaarchive.getCatalogId(),
					editor);
		}
	...
	public void removeCategoryFromAsset(WebPageRequest inPageRequest)
		...
		{
			CompositeAsset composite = (CompositeAsset) inPageRequest.getSessionValue(assetid);
	...
	public void addCategoryToAsset(WebPageRequest inPageRequest) throws Exception 
	{
		...
		for (int i = 0; i < add.length; i++)
		{
			...
			{
				CompositeAsset composite = (CompositeAsset) inPageRequest.getSessionValue(asset.getId());
	 ...
	 * TODO: Shouldn't this go into AssetEditModule ?
	 * @param inRequest
