 ...
 * 	protected String findSourcePath(WebPageRequest inReq) throws Exception
	{
		if(!(inReq.getPageValue("asset") instanceof Asset))
		{
			return null;
		}
		...
		if( sourcePath == null)
		{
			...
			if (assetid != null)
			{
				return archive.getAssetSearcher().idToPath(assetid);
			}
	...
	*/
	...
	public void openAssetViewPermissions(WebPageRequest inReq) throws Exception
	{
		...
		String path = "/" + asset.getCatalogId() + "/assets/"  + asset.getSourcePath() + "/";
		...
		if( viewasset != null && viewasset.booleanValue())
		{
			...
			archive.getAssetSecurityArchive().grantAccess(archive, page, "viewasset");
