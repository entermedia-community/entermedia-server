...
{
	private static final Log log = LogFactory.getLog(ImageMagickImageCreator.class);
	...
	protected ConvertResult createOutput(MediaArchive inArchive, Asset inAsset, Page inOutFile, ConvertInstructions inStructions) {
		...
		if(ext == null && input !=null) {
			//try and get the fileformat from exif, we do this here because the fileformat set on the asset
			//may be different than the fileformat of the imagefile property
			Asset thumbdata = new Asset();
			AssetUtilities utils = inArchive.getAssetImporter().getAssetUtilities();
