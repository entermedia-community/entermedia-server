...
public class ImageMagickImageCreator extends BaseImageCreator {
	private static final Log log = LogFactory
			.getLog(ImageMagickImageCreator.class);

	public boolean canReadIn(MediaArchive inArchive, String inInput) {
		return !inInput.equals("mp3") || canPreProcess(inArchive, inInput); // Can
																			// read
																			// in
																			// most
																			// any
	...
	public ConvertResult convert(MediaArchive inArchive, Asset inAsset,
			Page inOutFile, ConvertInstructions inStructions)
			throws OpenEditException {
		if (inStructions.isWatermark()) {
			inStructions.setWatermark(false);
			String outputPath = populateOutputPath(inArchive, inStructions); // now
																				// does
																				// not
																				// contain
																				// wm
			...
			if (!outputPage.exists()) {
				createOutput(inArchive, inAsset, outputPage, inStructions); // this
																			// will
																			// create
																			// smaller
																			// version
	...
	protected ConvertResult createOutput(MediaArchive inArchive, Asset inAsset,
			Page inOutFile, ConvertInstructions inStructions) {
		...
		if (inStructions.isWatermark()) {
			Page inputPage = getPageManager().getPage(
					inStructions.getAssetSourcePath());
			if (inputPage == null || !inputPage.exists()) {
				result.setOk(false);
				return result;
			}
			String fullInputPath = inputPage.getContentItem().getAbsolutePath();
			return applyWaterMark(inArchive, fullInputPath, outputpath,
					inStructions);
		}

		Page input = null;
		boolean autorotate = true; // If we already have a smaller version we
		...
		if (inStructions.getMaxScaledSize() != null && offset == null && inStructions.getPageNumber() == 1 ) {
			if (input == null
				...
				input = getPageManager().getPage(
						"/WEB-INF/data" + inArchive.getCatalogHome()
								+ "/generated/" + inAsset.getSourcePath()
								+ "/image300x300.jpg");
				if (!input.exists()) {
					input = null;
				} else {
			...
			if (input == null
					&& inStructions.getMaxScaledSize().getWidth() < 800) {
				input = getPageManager().getPage(
						"/WEB-INF/data" + inArchive.getCatalogHome()
								+ "/generated/" + inAsset.getSourcePath()
								+ "/image800x600.jpg");
		...
		// Look over to see if there is a creator that can do a better job of
		// reading in this type
		...
		if (ext == null && input != null) {
			// try and get the fileformat from exif, we do this here because the
			// fileformat set on the asset
			// may be different than the fileformat of the imagefile property
			Asset thumbdata = new Asset();
			AssetUtilities utils = inArchive.getAssetImporter()
					.getAssetUtilities();
			MetaDataReader reader = utils.getMetaDataReader();
			File inputFile = new File(input.getContentItem().getAbsolutePath());
			reader.populateAsset(inArchive, inputFile, thumbdata);
			ext = thumbdata.get("fileformat");
		}
		...
		if (ext != null) {
			...
			if (preprocessor != null) {
				// This will output a native format. First one wins. it is not a
				// loop.
				String tmppath = preprocessor.populateOutputPath(inArchive,
						inStructions);
				Page out = getPageManager().getPage(tmppath);
				if (!out.exists() || out.getContentItem().getLength() == 0) {
					// Create
					ConvertResult tmpresult = preprocessor.convert(inArchive,
							inAsset, out, inStructions);
					if (!tmpresult.isOk()) {
						return tmpresult;
					}
					// if( !inStructions.isWatermark() &&
					// out.getPath().equals(inOutFile.getPath())) //this is the
					// same file we originally wanted
					// {
					// //return tmpresult;
					// }
					input = out;
				} else if (input == null) {
					input = out; // we are looking for a working format to use
									// as input
				}
			}
		}
		...
		if (inStructions.getMaxScaledSize() != null) {
			// be aware ImageMagick writes to a tmp file with a larger version
			// of the file before it is finished
			if ("eps".equalsIgnoreCase(ext) || "pdf".equalsIgnoreCase(ext)
					|| "ai".equalsIgnoreCase(ext)) {
				// check input width
			...
			// we need to rotate this before we start otherwise the width and
			// heights might be flipped. This is only 5% slower
			// since we now strip all metadata from resulting images the
				...
				if (rotation != null) {
					try // TODO: Use a local lookup map
					{
						Searcher searcher = inArchive.getSearcherManager()
								.getSearcher(inArchive.getCatalogId(),
										"imageorientation");
		...
		// TODO: Is this needed any more? Seems that ImageMagik will use the
		// orientation flag that is built in
		if (autorotate && inStructions.getRotation() != 0
				&& inStructions.getRotation() != 360) {
			com.add("-rotate");
	...
	protected List<String> createCommand(File inFile,
	...
	public void resizeThumbnailImage(File inFile, File inOutFile,
			ConvertInstructions inStructions) throws Exception {
		...
		if (runExec("convert", com)) {
			log.info("Resize complete in:"
					+ (System.currentTimeMillis() - start) + " "
					+ inOutFile.getName());
		}
	}

	public List<String> getConvertCommand(File inIn, File inOut,
			int inPageNumber) {
		List<String> com = new ArrayList<String>();
	...
	/*
	 * protected File getOriginalDocument(MediaArchive inArchive,
	 * ConvertInstructions inStructions, Asset asset) { String original = null;
	 * if (inStructions.getSourceFile() != null) { original =
	 * inArchive.getOriginalFileManager
	 * ().getFilePath(inStructions.getSourceFile(), asset); } else { original =
	 * inArchive.getOriginalFileManager().getOriginalFilePath(asset); }
	 * if(original == null) { return null; //we haven't uploaded anything yet }
	 * 
	 * String extension = PathUtilities.extractPageType(original);
	 * inStructions.setInputType(extension);
	 * 
	 * String orientation = asset.get("imageorientation"); if (orientation !=
	 * null) { Data ed = (Data)
	 * inArchive.getSearcherManager().getSearcher(asset.getCatalogId(),
	 * "imageorientation").searchById(orientation); if (ed != null) { String
	 * rotation = ed.get("rotation"); if (rotation != null &&
	 * !rotation.equals("")) {
	 * inStructions.setRotation(Integer.parseInt(rotation)); } } }
	 * 
	 * File input = new File(original); return input; }
	 */
	public ConvertResult applyWaterMark(MediaArchive inArchive,
			String inInputAbsPath, String inOutputAbsPath,
			ConvertInstructions inStructions) {
	...
	/*
	 * private File makeWaterMark(ConvertInstructions inStructions, File input,
	 * String watermarkPath) { File watermarked = null; ContentItem stub =
	 * getPageManager().getRepository().getStub(watermarkPath); watermarked =
	 * new File(stub.getAbsolutePath());
	 * 
	 * if (watermarked.exists() && watermarked.length() > 0) { return
	 * watermarked; } else { watermarked.getParentFile().mkdirs(); try {
	 * watermarked.createNewFile(); } catch (IOException e) { throw new
	 * OpenEditException(e); }
	 * 
	 * // watermark it MediaCreator convert =
	 * getConverterByFileFormat(inStructions.getInputType());
	 * convert.applyWaterMark(getMediaArchive(), input, watermarked,
	 * inStructions); return watermarked; } }
	 */
}
