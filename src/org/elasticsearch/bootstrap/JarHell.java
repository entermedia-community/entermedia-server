 ...
 * <li>Checks any {@code X-Compile-Target-JDK} value in the jar manifest is compatible with current
 * JRE</li>
 * <li>Checks any {@code X-Compile-Elasticsearch-Version} value in the jar manifest is compatible
 * with the current ES</li>
 * </ul>
 */
    ...
    public static void checkVersionFormat(String targetVersion)
    {
        if (!JavaVersion.isValid(targetVersion))
        {
            throw new IllegalStateException(
                String.format(Locale.ROOT, "version string must be a sequence of nonnegative decimal integers separated by \".\"'s and may have leading zeros but was %s", targetVersion));
        }
    }

    /**
     * Checks that the java specification version {@code targetVersion} required by {@code resource} is
     * compatible with the current installation.
     */
