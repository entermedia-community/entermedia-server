...
public abstract class HtmlTreeRenderer extends BaseTreeRenderer {
	 ...
	 * 
	 * @param inIconHome the root directory for icons. If <code>null</code> defaults
	 *                   to <code>/openedit/images/tree/</code>.
	 * 
	 */
	...
	public String renderAsString() {
		...
		// Amazingly... In IE6.0 you cannot name your div id the same as a globally
		// defined javascript variable
	...
	protected void renderLeaf(int level, StringBuffer js, String id, String name, String path, boolean select,
			String icon, int wide) {
		// js.append("\n </div>\n");
		// if (entire)
		// {
		// }
		...
		js.append("\n<div class='" + (select ? "dTreeNodeSel " : "") + "treerowtext'  id='" + getWebTree().getName()
				+ "treerowtext" + id + "'  path='" + path + "' nodeid='" + id + "' >");
		...
		// }
		// indent(js,level+2);
		js.append("<a title='" + path + "' href='#" + path + "' onclick=\"return " + getWebTree().getName()
				+ ".jumpToNode('" + path + "','" + id + "');\"  >");
		// js.append("<img src='" + getHome() + getIconHome() + icon + "page.png'
		// class='treeicon' id='" + getWebTree().getName() + "ipageTree" + id + "'
		// alt='' />" + name + "</a></div></div></div>");
		js.append(name + "</a></div></div></div>");
	}

	protected void renderRow(Object inNode, StringBuffer js, String id, String name, String path, String icon,
			boolean expanded, boolean shouldexpand, boolean isroot, int level, int padding, boolean select) {
		js.append("<div  id='" + getWebTree().getName() + "treerow" + id + "'  >");
		js.append("<div class='" + (select ? "dTreeNodeSel " : "") + "treerowtext'  id='" + getWebTree().getName()
				+ "treerowtext" + id + "'  path='" + path + "' nodeid='" + id + "'>");
		...
		if ((!isroot && isRenderLeaves() && getWebTree().getModel().hasChildren(inNode))
			...
			js.append("<a class='node' href='#" + path + "' onclick=\"return " + getWebTree().getName()
					+ ".toggleNode('" + path + "','");
			...
			if (expanded) {
				...
				if (inNode != getWebTree().getModel().getRoot()) {
					js.append(
							"<img alt='' src='" + getHome() + getIconHome() + "down.png' id='jpageTree" + id + "' />");
				}
				// js.append("<img alt='' class='treeicon' src='" + getHome() + getIconHome() +
				// icon + "folderopen.png' id='ipageTree" + id + "' />" );
			} else {
				shouldexpand = true;
				if (inNode != getWebTree().getModel().getRoot()) {
					js.append(
							"<img alt='' src='" + getHome() + getIconHome() + "right.png' id='jpageTree" + id + "' />");
				}
			}
			js.append("</a>");
		} else {
