...
public interface WebTreeModel {
	 ...
	 * parent's child
	 * array. <code>parent</code> must be a node previously obtained from this data
	 * source. This
	 * should not return <code>null</code> if <code>index</code> is a valid index
	 * for
	 ...
	 * is a leaf or
	 * if it has no children. <code>parent</code> must be a node previously obtained
	 * from this
	 * data source.
	 *
	 * @param parent A node in the tree, obtained from this data source
	 ...
	 * this method to
	 * return <code>false</code> even if <code>node</code> has no children. A
	 * directory in a
	...
	/**
	 * Returns the root of the tree. Returns <code>null</code> only if the tree has
	 * no nodes.
	 *
