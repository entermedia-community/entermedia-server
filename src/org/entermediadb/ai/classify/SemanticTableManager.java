...
public class SemanticTableManager extends BaseAiManager implements CatalogEnabled {
	...
	public SemanticConfig getSemanticInstructions() {
		...
		SemanticConfig instructions = (SemanticConfig) getMediaArchive().getCacheManager()
				.get("semantictopicsinstructions", sematicsettingsid);
	...
	protected void indexData(ScriptLogger inLog, SemanticConfig instruction,
			Collection<? extends MultiValued> inRecords) {
		Collection<MultiValued> createdVectors = new ArrayList();
	...
	public int index(SemanticConfig inStructions, Collection<? extends MultiValued> inEntities,
			Collection<MultiValued> createdVectors) // Page of data
	{
		...
		HitTracker existingvectors = getMediaArchive().query(inStructions.getSearchType()).orgroup("dataid", inEntities)
				.search();
	...
	protected Collection<MultiValued> extractVectors(SemanticConfig inStructions, String inModuleId,
			List<MultiValued> entitiestoscan) throws Exception {
	...
	public Collection<String> createSemanticValues(LlmConnection llmconnection, MultiValued inConfig, String inModuleId,
			MultiValued inData) {
