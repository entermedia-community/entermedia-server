...
public class EmbeddingManager extends BaseAiManager {
	...
	public SmartCreatorManager getSmartCreatorManager() {
		return (SmartCreatorManager) getMediaArchive().getModuleManager().getBean(getCatalogId(), "smartCreatorManager",
				true);
	}
	...
	protected void processOneEntity(ScriptLogger inLogger, MultiValued inEntity, String inModuleId) throws Exception {
		...
		if (method != null && method.equals("smartcreator")) {
			...
			Collection pages = getMediaArchive().query(inModuleId + "page").exact(inModuleId, inEntity.getId())
					.search();
	...
	/*
	 * protected void embedSections(ScriptLogger inLog, String inSearchtype,
	 * Collection<Data>
	 * inToprecess, Searcher inPageSearcher) { for (Iterator iterator =
	 * inToprecess.iterator();
	 * iterator.hasNext();) { long start = System.currentTimeMillis();
	 * 
	 * MultiValued parententity = (MultiValued) iterator.next();
	 * 
	 * 
	 * embedSectionData(inLog, parententity, inSearchtype); inLog.info("Embedded "+
	 * inSearchtype +
	 * " in " + (System.currentTimeMillis() - start) + " ms"); } }
	 */
	/*
	 * protected void embedSectionData(ScriptLogger inLogger, Data inEntity, String
	 * searchtype) {
	 * JSONObject documentdata = new JSONObject(); documentdata.put("doc_id",
	 * searchtype + "_" +
	 * inEntity.getId());
	 * 
	 * SmartCreatorSession smartcreatorsession =
	 * getSmartCreatorManager().loadSections(searchtype,
	 * inEntity.getId()); Collection allpages = new ArrayList();
	 * 
	 * 
	 * for (Data section : smartcreatorsession.getSections()) { StringBuilder
	 * sectiontext = new
	 * StringBuilder(); Collection<Data> sectionComponents =
	 * smartcreatorsession.getSectionComponents(section.getId()); for (Data
	 * component:
	 * sectionComponents) { if( "paragraph".equals(component.get("componenttype")))
	 * { String textcontent
	 * = component.get("content"); if (textcontent != null &&
	 * !textcontent.isEmpty()) {
	 * sectiontext.append(textcontent).append("\n"); } } }
	 * 
	 * JSONObject pagedata = new JSONObject(); pagedata.put("page_id", searchtype +
	 * "page_" +
	...
	protected void embedEntityWithPages(ScriptLogger inLog, String inSearchtype, Collection<Data> inToprecess,
			Searcher inPageSearcher) {
		for (Iterator iterator = inToprecess.iterator(); iterator.hasNext();) {
			long start = System.currentTimeMillis();

			MultiValued document = (MultiValued) iterator.next();

			Collection pages = getMediaArchive().query(inSearchtype + "page").exact(inSearchtype, document.getId())
					.search(); // TODO: Check a view?
	...
	protected void embedEntity(ScriptLogger inLog, String inSearchtype, Collection<Data> inToprecess,
			Searcher inPageSearcher) {
		for (Iterator iterator = inToprecess.iterator(); iterator.hasNext();) {
			long start = System.currentTimeMillis();
	...
	public void embedEntityData(ScriptLogger inLogger, MultiValued inEntity, String searchtype) {
		...
		if (entityembeddingstatus == null || !"embedded".equals(entityembeddingstatus)) {
			...
			if (detail != null) {
				...
				Collection detailsfields = getMediaArchive().getSearcher(searchtype)
						.getDetailsForView(searchtype + "general");
				...
				String templatepath = getMediaArchive().getMediaDbId()
						+ "/ai/default/calls/commons/context_fields.json";
	...
	protected void embedBlogs(ScriptLogger inLog, String inSearchtype, Collection<Data> inToprecess,
			Searcher inPageSearcher) {
	...
	public void processRagResponseWithSource(AgentContext inAgentContext, JSONObject ragresponse,
