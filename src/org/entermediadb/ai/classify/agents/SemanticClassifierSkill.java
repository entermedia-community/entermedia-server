...
public class SemanticClassifierSkill extends BaseSkill {
	public SemanticClassifierManager getSemanticClassifierManager() {
		SemanticClassifierManager manager = (SemanticClassifierManager) getMediaArchive()
				.getBean("semanticClassifierManager");
		return manager;
	}
	...
	public void process(AgentContext inContext) {
		...
		if (inAssets != null && !inAssets.isEmpty()) {
			...
			informatic.info("Classified: " + inAssets.size() + " assets took "
		...
		if (inRecords != null && !inRecords.isEmpty()) {
			...
			long duration = System.currentTimeMillis() - startTime;
			informatic.info("Classified: " + inRecords.size() + " records took "
					+ (duration > 1000L ? duration / 1000L + "s" : duration + " ms"));
		}
