...
public class GeminiConnection extends BaseLlmConnection implements CatalogEnabled, LlmConnection {
	 ...
	 * // String endpoint = "http://localhost:3000/generations"; // for local
	 * testing
	 * 
	 * HttpPost method = new HttpPost(endpoint);
	 * method.addHeader("x-goog-api-key", getApiKey());
	 ...
	 * 
	 * String contentPath = "/" + archive.getMediaDbId() +
	 * "/ai/gemini/createdialog/systemmessage/" + inFunction + ".html";
	 * boolean contentExists =
	 * archive.getPageManager().getPage(contentPath).exists();
	 * if (!contentExists)
	 * {
	 * contentPath = "/" + archive.getCatalogId() +
	 * "/ai/gemini/createdialog/systemmessage/" + inFunction + ".html";
	 * contentExists = archive.getPageManager().getPage(contentPath).exists();
	 * }
	 * if (!contentExists)
	 * {
	 * throw new OpenEditException("Requested Content Does Not Exist in MEdiaDB or
	 * Catatlog:" + inFunction);
	 * }
	 * 
	 ...
	 * if (inFunction != null)
	 * {
	 * String functionPath = "/" + archive.getMediaDbId() +
	 * "/ai/gemini/createdialog/functions/" + inFunction + ".json";
	 * boolean functionExists =
	 * archive.getPageManager().getPage(functionPath).exists();
	 * if (!functionExists)
	 * {
	 * functionPath = "/" + archive.getCatalogId() +
	 * "/ai/gemini/createdialog/functions/" + inFunction + ".json";
	 * functionExists = archive.getPageManager().getPage(functionPath).exists();
	 * }
	 * if (!functionExists)
	 * {
	 * throw new OpenEditException("Requested Function Does Not Exist in MEdiaDB or
	 * Catatlog:" + inFunction);
	 * }
	 * 
	 ...
	 * }
	 * 
	 * public LlmResponse callClassifyFunction(Map params, String inFunction, String
	 * inBase64Image)
	 * {
	 ...
	 * 
	 * public LlmResponse callClassifyFunction(Map params, String inFunction, String
	 * inBase64Image, String textContent)
	 * {
	 * MediaArchive archive = getMediaArchive();
	 * 
	 ...
	 * String systemMessage = loadInputFromTemplate("/" +
	 * getMediaArchive().getMediaDbId() +
	 * "/ai/default/systemmessage/"+inFunction+".html");
	 ...
	 * if (inFunction != null)
	 * {
	 * String templatepath = "/" + archive.getMediaDbId() +
	 * "/ai/gemini/classify/functions/" + inFunction + ".json";
	 * 
	 * Page defpage = archive.getPageManager().getPage(templatepath);
	 * 
	 * if (!defpage.exists())
	 * {
	 * templatepath = "/" + archive.getCatalogId() +
	 * "/ai/gemini/classify/functions/" + inFunction + ".json";
	 ...
	 * 
	 * @Override
	 *           public LlmResponse callStructuredOutputList(String inStructureName,
	 *           Map inParams)
	 *           {
	 *           inParams.put("model", getModelName());
	 * 
	 *           String inStructure = loadInputFromTemplate("/" +
	 *           getMediaArchive().getMediaDbId() +
	 *           "/ai/gemini/classify/structures/" + inStructureName + ".json",
	 *           inParams);
	 * 
	 *           JSONParser parser = new JSONParser();
	 ...
	 *           method.addHeader("x-goog-api-key", getApiKey());
	 *           method.setHeader("Content-Type", "application/json");
	 *           method.setEntity(new StringEntity(structureDef.toJSONString(),
	 *           StandardCharsets.UTF_8));
	 * 
	 *           CloseableHttpResponse resp = getConnection().sharedExecute(method);
	 * 
	 ...
	 *           if (resp.getStatusLine().getStatusCode() != 200)
	 *           {
	 *           throw new OpenEditException("Gemini error: " +
	 *           resp.getStatusLine());
	 *           }
	 * 
	 *           JSONObject json = (JSONObject) parser.parse(new
	 *           StringReader(EntityUtils.toString(resp.getEntity(),
	 *           StandardCharsets.UTF_8)));
	 * 
	 *           log.info("Returned: " + json.toJSONString());
	 * 
	 ...
	 * 
	 * @Override
	 *           public LlmResponse callOCRFunction(Map inParams, String
	 *           inOCRInstruction, String inBase64Image)
	 *           {
	 *           throw new OpenEditException("Not implemented yet. Only available in
	 *           Llama connection.");
	 *           }
	 * 
	 * @Override
