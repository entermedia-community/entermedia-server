 ...
 * The Agent will be
 * responsible for executing the task and updating the status of the task. The
 * TaskManager will be
 * responsible for scheduling the tasks and keeping track of the status of the
 * tasks. The
 * TaskManager will also be responsible for providing a UI for the tasks and
 * allowing users to
 * interact with the tasks. A Task can be a big one or small ones. For example,
 * a big task can be
 * "Classify all assets in the system" and a small task can be "Classify asset
 * 12345". The
 * TaskManager will be responsible for breaking down big tasks into smaller
 * tasks and scheduling
 * them accordingly. My plan is to have a UI where each Task can be seen and
 * assigned to a Agent.
 * The Agent will be responsible for executing the task and updating the status
 * of the task. The
 * TaskManager will be responsible for scheduling the tasks and keeping track of
 * the status of the
 * tasks. The TaskManager will also be responsible for providing a UI for the
 * tasks and allowing
 * users to interact with the tasks. A Task can be a big one or small ones. For
 * example, a big task
 * can be "Classify all assets in the system" and a small task can be "Classify
 * asset 12345". The
 * TaskManager will be responsible for breaking down big tasks into smaller
 * tasks and scheduling
 * them accordingly.
 * 
 * A task can retry a few times if it fails. It will have a retry count and a
 * retry delay. The
 * TaskManager will be responsible for retrying the task if it fails and
 * updating the status of the
 * task accordingly.
 * 
 * Tasks will have Steps that are connected to AI Functions. The functions have
 * their own
 * configuration. The TaskManager will be responsible for executing the steps in
 * order and passing
 * the output of one step to the next step. The TaskManager will also be
 * responsible for handling
 * errors and retrying steps if they fail.
 * 
 * Once the Tasks are identified there will be a set of Agents that look over
 * the tasks and execute
 * them. The Agents will be responsible for executing the task and updating the
 * status of the tasks.
 * 
 */

public class InformaticsProcessorSkill extends BaseSkill {
	private static final Log log = LogFactory.getLog(InformaticsProcessorSkill.class);

	public InformaticsProcessorManager getInformaticsProcessorManager() {
		InformaticsProcessorManager manager = (InformaticsProcessorManager) getMediaArchive()
	...
	public void process(AgentContext inContext) {
		if (!(inContext instanceof InformaticsContext)) {
			log.error(inContext.getCatalogId() + " Can't process Agent: " + inContext.getCurrentAgentEnable()
					+ " wrong context");
			return;
		...
		log.info(inContext.getCatalogId() + " Processing Informatics Agent: " + inContext.getCurrentAgentEnable()
				+ " in Scenario: " + inContext.getCurrentScenerio());
	...
	public void processPendingAssets(InformaticsContext inContext) {
		...
		if (inPendingAssets != null && !inPendingAssets.isEmpty()) {
			// inLog.info("Adding metadata to: " + pendingrecords.size() + " assets in
			// category: " + categoryid
	...
	protected void processPendingRecords(InformaticsContext inContext) {
		...
		if (!pendingrecords.isEmpty()) {
			...
			for (int i = 0; i < pendingrecords.getTotalPages(); i++) {
				...
				if (validhits.isEmpty()) {
					// inContext.info("No Entities to Process in modules: " + ids + " | Search
					// Query: "+
		...
		if (validhits.isEmpty()) {
			...
			// inContext.info("Processing " + validhits.size() + " Entities Informatics
			// Complete.");
		}
	}
