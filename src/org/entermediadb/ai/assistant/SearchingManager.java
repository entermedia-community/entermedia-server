...
public class SearchingManager extends BaseAiManager implements ChatMessageHandler {
	...
	public void searchTables(WebPageRequest inReq, AiSearch inAiSearchParams) {
		...
		if (step1 != null && step2 != null) {
			...
			HitTracker foundhits = getMediaArchive().query(step1.getModule().getId())
					.freeform("description", step1Keyword).search();
			...
			QueryBuilder search = getMediaArchive().query(step2.getModule().getId()).named("assitedsearch")
					.orgroup(step1.getModule().getId(), ids);
	...
	public Collection<SemanticAction> createPossibleFunctionParameters(ScriptLogger inLog) {
		...
		for (Iterator iterator = schema.getModules().iterator(); iterator.hasNext();) {
			...
			Collection existing = embedsearcher.query().exact("aifunction", "search_tables")
					.exact("parentmodule", parentmodule.getId()).search();
			if (!existing.isEmpty()) {
	...
	protected void loadSearchParts(AgentContext inAgentContext, JSONObject structure, String messageText) {
		...
		if (search.getStep1().getTargetTable() == null) {
			...
			if (search.getStep2() != null) {
				text = text + " for " + search.getStep2().getTargetTable() + " in "
						+ search.getStep1().getTargetTable();
	...
	public void processAISearchArgs(JSONObject airesults, Data inEmbeddingMatch, AgentContext inContext,
			AiSearch inAiSearch) {
	...
	public void semanticSearch(WebPageRequest inReq) {
		...
		Map<String, Collection<String>> relatedEntityIds = getSemanticTopicManager().search(semanticquery,
				excludeEntityIds, excludeAssetIds);
	...
	public SemanticClassifierManager getSemanticTopicManager() {
		if (fieldSemanticTopicManager == null) {
			fieldSemanticTopicManager = (SemanticClassifierManager) getModuleManager().getBean(getCatalogId(),
					"semanticClassifierManager", false);
	...
	public void rescanSearchCategories(ScriptLogger inLogger) {
		...
		for (Iterator iterator = tracker.iterator(); iterator.hasNext();) {
			...
			for (Iterator iterator2 = bytype.keySet().iterator(); iterator2.hasNext();) {
				...
				Collection addedentites = getMediaArchive().query(moduleid).ids(ids)
						.not("searchcategory", searchcategory.getId()).search();
	...
	public Collection<Map> getRelatedRecords(String inEntityModuleId, String inEntityId) {
		Collection detailsviews = getMediaArchive().query("view").exact("moduleid", inEntityModuleId)
	...
	public Data getRecord(String inEntityModuleId, String inEntityId, String inListId, String inRecordId) {
		Data record = getMediaArchive().query(inListId).exact(inEntityModuleId, inEntityId).exact("id", inRecordId)
				.searchOne();
		return record;
	}
}
