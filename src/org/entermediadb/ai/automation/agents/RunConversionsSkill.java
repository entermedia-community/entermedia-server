...
public class RunConversionsSkill extends BaseSkill {
	@Override
	public void process(AgentContext inContext) {
		// TODO: Use a single thread for this?

		QueueManager queueManager = (QueueManager) getModuleManager().getBean(getMediaArchive().getCatalogId(),
				"queueManager");
		queueManager.checkQueue();

		if (queueManager.getTotalPending() > 0) {
			inContext.info("Total Pending Tasks: " + queueManager.getTotalPending() + " Threads running: "
					+ queueManager.runningProcesses() + " of " + queueManager.getMaxProcessors());
		}
