...
public class GoogleManager implements CatalogEnabled {
		...
		try {
			if (resp.getStatusLine().getStatusCode() != 200) {
				log.info("Google Server error returned " + resp.getStatusLine().getStatusCode() + " "
						+ resp.getStatusLine() + " File: " + inAsset.getName());
				 ...
				 * ?apix_params=%7B%
				 * 22fileId%22%3A%2216LawVTbF9eiC7sGghSNwBxRJwQ36das%22%2C%22fields%22%3A%22*%22
				 * %7D
				 */
	...
	public JSONObject processImage(HttpSharedConnection connection, Asset inAsset) {
		...
		String input = "/WEB-INF/data/" + archive.getCatalogId() + "/generated/" + inAsset.getSourcePath()
				+ "/image3000x3000.jpg";

		Page inputpage = archive.getPageManager().getPage(input);
		if (!inputpage.exists()) {
			input = "/WEB-INF/data/" + archive.getCatalogId() + "/generated/" + inAsset.getSourcePath()
					+ "/image3000x3000.png";
	...
	public JSONObject uploadToBucket(Data inAuthInfo, String bucket, ContentItem inItem, JSONObject inMetadata)
			throws Exception {
	...
	// getAccessToken(getMediaArchive().getData("oauthprovider", "google")));
	//
	...
	public void notifyTopic(final String inChannel, final User inUser, final String inSubject, final String inMessage,
			final Map inExtraData) {
	...
	public Map<String, String> getTokenDetails(String token) {
		...
		JSONObject resp = getConnection()
				.getJson("https://www.googleapis.com/oauth2/v1/tokeninfo?access_token=" + token);
		if (resp != null) {

			/**
			 * { "issued_to":
			 * "279466694094-jo658skoqembq8p6nd5fqsl2t2p15lj0.apps.googleusercontent.com",
			 * "audience":
			 * "279466694094-jo658skoqembq8p6nd5fqsl2t2p15lj0.apps.googleusercontent.com",
			 * "user_id": "101451826132682989401", "scope":
			 * "https://www.googleapis.com/auth/userinfo.email
			 * openid https://www.googleapis.com/auth/userinfo.profile", "expires_in": 1306,
			 * "email":
			 * "cburkey@openedit.org", "verified_email": true, "access_type": "online" }
			 */
			return resp;
		}
	...
	protected void createFirebaseUser(String apikey, User inUser) {
		...
		if (createresp.getStatusLine().getStatusCode() != 200) {
			...
			log.error("Could not create user in firebase. Please manually delete existing user " + inUser.getEmail()
					+ " " + returned);
			return;
		}
		JSONObject json = getConnection().parseMap(createresp);
	 ...
	 * 
	 * 
	 * function getLatLngFromString(ll) { var latlng = ll.split(/, ?/) return new
	 * google.maps.LatLng(parseFloat(latlng[0]), parseFloat(latlng[1])); }
	 * 
	 * var geocoder;
	 * 
	 * initialize = function() { var parentheight =
	 * $(".assetpanel-geomap").height(); if (typeof
	 * parentheight !== 'undefined') { var mapheight =
	 * (jQuery(".assetpanel-geomap").height()-110);
	 * jQuery('#geo_point_map').height(mapheight); } var centerloc =
	 * getLatLngFromString('39.025818619038375,-95.03089171562499'); var map = new
	 * google.maps.Map(jQuery('#geo_point_map')[0], { zoom: 2 , center:centerloc,
	 * mapTypeControl: true,
	 * mapTypeControlOptions: { style:
	 * google.maps.MapTypeControlStyle.DROPDOWN_MENU, mapTypeIds:
	 * ['roadmap', 'terrain'] } });
	 * 
	 * var marker = new google.maps.Marker({ position:
	 * getLatLngFromString('$lat,$lng'), draggable:
	 * true, bounds: true, map: map, title: '{"en":"Screenshot from 2023-06-22
	 * 15-53-59.png"}' });
	 * 
	 * 
	 * google.maps.event.addListener(marker, 'dragend', function(event) {
	 * console.log(marker.getPosition());
	 * jQuery('#geo_point_latandlong').val(marker.getPosition().lat()+ "," +
	 * marker.getPosition().lng()); });
	 * 
	 * 
	 * jQuery('#geo_point_search').click(dosearch);
	 * 
	 * jQuery('#geo_point_address').keypress(function(e) { if (e.keyCode == 13) {
	 * dosearch() } });
	 * 
	 ...
	 * }
	 * 
	 * dosearch = function () { var address = jQuery('#geo_point_address').val();
	 * var postalCode =
	 * jQuery('#geo_point_postalCode').val(); if (address!=null) {
	 * $('#geo_point_map').gmap('search', {
	 * 'address': address }, function(results, status) { if ( status === 'OK' ) {
	 * $('#geo_point_map').gmap('clear', 'markers');
	 * $('#geo_point_map').gmap('addMarker', { 'position':
	 * results[0].geometry.location, 'draggable': true, 'bounds': true
	 * }).dragend(function(event) {
	 * jQuery('#geo_point_latandlong').val(event.latLng.lat() + "," +
	 * event.latLng.lng()); } );
	 * 
	 * $('#geo_point_map').gmap('get', 'map').panTo(results[0].geometry.location);
	 * $('#geo_point_map').gmap('option', 'zoom', 12);
	 * jQuery('#geo_point_latandlong').val(results[0].geometry.location.lat() + ","
	 * +
	 * results[0].geometry.location.lng());
	 * 
	 * } }); } }
	 * 
	 * initialize(); //Onload? $(window).resize(function() { if (typeof map !==
	 * 'undefined') {
