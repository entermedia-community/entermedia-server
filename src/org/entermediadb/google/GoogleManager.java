...
public class GoogleManager implements CatalogEnabled
{
	...
	public Map<String, String> getTokenDetails(String token)
	{
		...
		if (resp != null)
		{
			 ...
			 * "user_id": "101451826132682989401", "scope": "https://www.googleapis.com/auth/userinfo.email
			 * openid https://www.googleapis.com/auth/userinfo.profile", "expires_in": 1306, "email":
	 ...
	 * var geocoder;
	 * 
	 * initialize = function() { var parentheight = $(".assetpanel-geomap").height(); if (typeof
	 * parentheight !== 'undefined') { var mapheight = (jQuery(".assetpanel-geomap").height()-110);
	 * jQuery('#geo_point_map').height(mapheight); } var centerloc =
	 * getLatLngFromString('39.025818619038375,-95.03089171562499'); var map = new
	 * google.maps.Map(jQuery('#geo_point_map')[0], { zoom: 2 , center:centerloc, mapTypeControl: true,
	 * mapTypeControlOptions: { style: google.maps.MapTypeControlStyle.DROPDOWN_MENU, mapTypeIds:
	 * ['roadmap', 'terrain'] } });
	 * 
	 * var marker = new google.maps.Marker({ position: getLatLngFromString('$lat,$lng'), draggable:
	 * true, bounds: true, map: map, title: '{"en":"Screenshot from 2023-06-22 15-53-59.png"}' });
	 * 
	 * 
	 ...
	 * 
	 * jQuery('#geo_point_address').keypress(function(e) { if (e.keyCode == 13) { dosearch() } });
	 * 
	 ...
	 * }
	 * 
	 * dosearch = function () { var address = jQuery('#geo_point_address').val(); var postalCode =
	 * jQuery('#geo_point_postalCode').val(); if (address!=null) { $('#geo_point_map').gmap('search', {
	 * 'address': address }, function(results, status) { if ( status === 'OK' ) {
	 * $('#geo_point_map').gmap('clear', 'markers'); $('#geo_point_map').gmap('addMarker', { 'position':
	 * results[0].geometry.location, 'draggable': true, 'bounds': true }).dragend(function(event) {
	 ...
	 * } }); } }
	 * 
	 * initialize(); //Onload? $(window).resize(function() { if (typeof map !== 'undefined') {
