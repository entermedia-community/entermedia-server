...
public class GoogleManager implements CatalogEnabled {
	...
	public Results listDriveFiles(Data authinfo, String inParentId) throws Exception {
		...
		String url = "https://www.googleapis.com/drive/v3/files?orderBy=" + URLEncoder.encode("modifiedTime desc,name")
	...
	public String getUserAccessToken(User user) throws Exception {
		...
		if (accesstoken == null || force) {
			...
			OAuthClientRequest request = OAuthClientRequest.tokenProvider(OAuthProviderType.GOOGLE)
					.setGrantType(GrantType.REFRESH_TOKEN).setRefreshToken(user.get("refreshtoken"))
					.setClientId(authinfo.get("clientid")).setClientSecret(authinfo.get("clientsecret"))
					.buildBodyMessage();
			OAuthClient oAuthClient = new OAuthClient(new URLConnectionClient());
			// Facebook is not fully compatible with OAuth 2.0 draft 10, access token
	...
	private void createAssetsIfNeeded(Data authinfo, Map inOnepage, Category category) throws Exception {
		...
		HitTracker existingassets = getMediaArchive().getAssetSearcher().query().orgroup("googleid", inOnepage.keySet())
				.search();
