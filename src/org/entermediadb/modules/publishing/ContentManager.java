...
public class ContentManager implements CatalogEnabled
{
	...
	public Data createFromLLM(WebPageRequest inReq, LLMManager inLlm, String inModel, Data inContentrequest) throws Exception
	{
		...
		String entitysourcetype = inContentrequest.get("entitysourcetype");
		String entityid = inContentrequest.get("entityid");
		Data parentmodule = getMediaArchive().getCachedData("module", entitysourcetype);//Exam Prep

		String view = inContentrequest.get("entitymoduleviewdata");
		...
		if (view != null)
		{
			...
			Searcher targetsearcher = getMediaArchive().getSearcher(submodsearchtype);

			inReq.putPageValue("parentmodule", parentmodule);
			inReq.putPageValue("targetmodule", targetmodule);
			inReq.putPageValue("contentrequest", inContentrequest);

			String template = inLlm.loadInputFromTemplate(inReq, "/" + archive.getMediaDbId() + "/gpt/templates/create_entity.html");
			JSONObject results = inLlm.callFunction(inReq, inModel, "create_entity", template, 0, 5000);

			Data child = targetsearcher.createNewData();
			targetsearcher.updateData(child, results);
			child.setValue("entity_date", new Date());
			child.setValue(entitysourcetype, entityid); // Lookup
			child.setValue("ai-functioncall", results.toJSONString());
			//No assets being created in this one.
			...
			//in this scenario we're creating a new top level entity
			Searcher targetsearcher = getMediaArchive().getSearcher(entitysourcetype);
			inReq.putPageValue("parentmodule", parentmodule);
			inReq.putPageValue("contentrequest", inContentrequest);
			String template = inLlm.loadInputFromTemplate(inReq, "/" + archive.getMediaDbId() + "/gpt/templates/create_entity.html");
			JSONObject results = inLlm.callFunction(inReq, inModel, "create_entity", template, 0, 5000);
