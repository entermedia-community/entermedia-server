...
public class TimelineManager {
	public Collection searchInVideo(MediaArchive inArchive, Asset inAsset, String searchby) {
		...
		// closed captions
		Collection tracks = captionsearcher.query().exact("assetid", inAsset.getId())
				.contains("captions.cliplabel", searchby).sort("timecodestart").search();
			...
			for (Iterator iterator = clips.iterator(); iterator.hasNext();) {
				...
				if (groupids != null && !groupids.isEmpty()) {
					for (Iterator iterator2 = groupids.iterator(); iterator2.hasNext();) {
						...
						if (group != null) {
							if (matchesText(searchby, group.getName()) || (group.get("facecounter") != null
									&& group.get("facecounter").contains(searchby))) {
								long num = Long.parseLong(start.toString());
