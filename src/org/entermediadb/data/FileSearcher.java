...
public class FileSearcher extends BaseSearcher implements PageAccessListener
{
	...
	public HitTracker searchFiles(String rootFolder, String field, String mask, String orderby)
	{
	...
	protected void collectHits(Page inRoot, List inResults, FileSearchQuery inQuery) throws ParseException
	{
		if (inRoot == null || inQuery == null || inRoot.getName() == null)
		{
			return;
		}
		if (inRoot.getName().equals(".versions") && !inQuery.isIncludeVersions())
		{
			return;
		}
		List childpaths = getPageManager().getChildrenPaths(inRoot.getPath(), inQuery.isIncludeFallback());
	...
	protected boolean passes(Page inPage, FileSearchQuery inQuery) throws ParseException
	{
		...
		for (Iterator iterator = inQuery.getTerms().iterator(); iterator.hasNext();)
		{
			...
			if ("betweendates".equals(term.getOperation()))
			{
				...
				if ("afterdate".equals(term.getOperation()))
				{
					...
					{
						Date before = inQuery.getDateFormat().parse((String) term.getValue("lowDate"));
						String id = term.getDetail().getId();// effectivedate
						String date = inPage.getProperty(id);
						if (date == null)
						{
