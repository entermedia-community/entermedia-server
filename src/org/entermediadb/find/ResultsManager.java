...
public class ResultsManager extends BaseManager {
	...
	/*
	 * public void organizeHits(WebPageRequest inReq, HitTracker inModuleHits,
	 * Collection pageOfHits, HitTracker inAssets)
	 * {
	 ...
	 * 
	 * Collection types =
	 * (Collection)inModuleHits.getSearchQuery().getValues("searchtypes");
	 ...
	 * }
	 * Map<String,Collection> bytypes = organizeHits(inReq, inModuleHits,
	 * pageOfHits.iterator(),targetsize, inAssets);
	 * 
	 * ArrayList foundmodules = processResults(inModuleHits, archive, targetsize,
	 * bytypes);
	 * 
	 ...
	 * // // for (Iterator iterator = copy.getTerms().iterator();
	 * iterator.hasNext();)
	 * // // {
	 ...
	 * archive.getAssetSearcher().cachedSearch(inReq,copy);
	 ...
	 * // //Integer mediasample =
	 * profile.getHitsPerPageForSearchType(hits.getSearchType());
	 ...
	 */
	/*
	 * protected ArrayList processResults(HitTracker hits, MediaArchive archive, int
	 * targetsize, Map<String, Collection> bytypes)
	 * {
	 * ArrayList foundmodules = new ArrayList();
	 * boolean foundasset = false;
	 * //See if we have enough from one page. If not then run searches to get some
	 * results
	 ...
	 * {
	 * //Only makes sense when someone searched for text. Otherwise we get all
	 * values from *
	 * String input = hits.getSearchQuery().getMainInput();
	 * if( input != null)
	 * {
	 * Collection moredata =
	 * loadMoreResults(archive,hits.getSearchQuery(),sourcetype, maxpossible);
	 * if(moredata != null)
	 * {
	 ...
	 * {
	 * sthits =
	 * searcher.query().hitsPerPage(maxpossible).exact("parentid","index").sort(
	 * "name").search();
	 * }
	 * else
	 * {
	 * sthits =
	 * searcher.query().hitsPerPage(maxpossible).all().sort("name").search();
	 * }
	...
	private Collection loadMoreResults(MediaArchive archive, SearchQuery inSearchQuery, String inSourcetype,
			int maxsize) {
		// search for more
	...
	public OrganizedResults loadOrganizedResults(WebPageRequest inReq, HitTracker inUnsorted,
			HitTracker inAssetunsorted) {
		OrganizedResults organizedresults = loadOrganizedResults(inReq, inUnsorted, inAssetunsorted, 0);
		return organizedresults;
	}

	public OrganizedResults loadOrganizedResults(WebPageRequest inReq, HitTracker inUnsortedEntities,
			HitTracker inAssetunsorted, int inSize) {
	...
	public Collection<String> loadUserSearchTypes(WebPageRequest inReq) {
		 ...
		 * (String)inReq.getUserProfile().getValue("defaultmodule");
