...
public class VideoEditor {
	...
	public void split(Page inVideo, double inStartTime, double inStopTime, OutputStream inOutput) throws IOException {
		...
		// Here we try to find a track that has sync samples. Since we can only start
		// decoding
		// at such a sample we SHOULD make sure that the start of the new fragment is
		// exactly
		// such a frame
		...
		for (Track track : tracks) {
			...
			if (inStopTime > -1) {
				...
				for (int i = 0; i < track.getSampleDurations().length; i++) {
					...
					currentSample++;
					// entry.getDelta() is the amount of time the current sample covers.
					// log.info("Checking " + track.getType() + " checking " + currentTime + ">=" +
					// inStopTime);
			...
			// log.info("Added track " + track.getType() + " " + startSample + " " +
			// endSample);
