...
public class McpGetHandler
{
	...
	public void listen()
	{
		...
		try
		{
			...
			//
			// String send = payload.toJSONString();
			...
			/*
			 * //res.flushBuffer(); //setClientWriter(getReq().getPageStreamer().getOutput().getWriter());
			 * //Send stuff these
			 * 
			 * //Now block forever? //getClientWriter().flush(); sendNothing(res); //while (isActive()) for (int
			 * i = 0; i < 10; i++) { Thread.sleep(1000); log.info("Sending"); sendNothing(res); }
	...
	private void sendNothing(HttpServletResponse res) throws Exception
	{
		...
		String send = payload.toJSONString();
		getClientWriter().write(send);
		getClientWriter().flush();
		// res.flushBuffer(); //? Needed?
