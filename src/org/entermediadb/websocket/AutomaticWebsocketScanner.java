...
public class AutomaticWebsocketScanner implements ServerApplicationConfig
{
	...
	public Set<ServerEndpointConfig> getEndpointConfigs(Set<Class<? extends Endpoint>> scanned)
	{
		...
		for (Iterator iterator = scanned.iterator(); iterator.hasNext();)
		{
			...
			if (path.startsWith("org.entermediadb") || path.startsWith("org.netevolved") || path.startsWith("org.thoughtframe"))
			{
				...
				ServerEndpointConfig conf = ServerEndpointConfig.Builder.create(endpoint, path) // FYI: Path Configured in OpenEditFilter
	...
	{
		/*
		 * log.info("loading classes " + scanned.size()); Set<Class<?>> results = new HashSet<Class<?>>();
		 * for (Class<?> clazz : scanned) { if (clazz.getPackage().getName().startsWith("org.entermediadb"))
		 * { if( javax.websocket.Endpoint.class.isAssignableFrom(clazz) ) { results.add(clazz);
		 * log.info("configured " + clazz.getPackage().getName()); } } } return results;
		 */
