...
public class ModuleManagerLoader extends ServerEndpointConfig.Configurator
{
	...
	public void modifyHandshake(ServerEndpointConfig config, HandshakeRequest request, HandshakeResponse response)
	{
		...
		if (httpSession != null)
		{
			...
			/*
			 * First, use a filter with the same path as the WebSocket. This will give you access to the
			 * HttpServletRequest and HttpSession. It also gives you a chance to create a session if it doesn't
			 * already exist (although in that case using an HTTP session at all seems dubious).
			 */
