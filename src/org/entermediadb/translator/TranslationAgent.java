...
public class TranslationAgent extends BaseAgent
{
	public TranslationManager getTranslationManager()
	{
		TranslationManager manager =
				(TranslationManager) getMediaArchive().getBean("translationManager");
		return manager;
	}
	...
	public void process(AgentContext inContext)
	{
		...
		if (pageofhits != null && !pageofhits.isEmpty())
		{
			...
			inContext.addContext("hits", pageofhits);
			super.process(inContext);

			/*
			 * MultiValued config = mycontext.getCurrentAgentEnable().getAgentData();
			 * getTranslationManager().translateDataFields(inContext.getScriptLogger(), config,
			 * pageofhits);
			 */
