	...
	public String createSourcePath(Data inAsset, String inUrl)
	{
		...
		if (sourcepath.startsWith("\\\\"))
		{
			...
			if (sourcepath.length() > 1 && sourcepath.charAt(1) == ':')
			{
				...
				if (sourcepath.length() > 3 && sourcepath.contains(":)"))
				{
		...
		/*
		 * String cumuluscatname = inAsset.get("externalid"); if (cumuluscatname != null &&
		 * cumuluscatname.length() > 0) { sourcepath = cumuluscatname + "/" + sourcepath; }
		 */
