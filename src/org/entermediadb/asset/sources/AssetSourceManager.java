...
public class AssetSourceManager implements CatalogEnabled {
	 ...
	 * 
	 * @see
	 * org.entermediadb.asset.Bob#getOriginalDocumentStream(org.entermediadb.asset.
	 * Asset)
	 */
	...
	public Collection getAssetSources() {
		if (fieldAssetSources == null) {
			...
			Collection hits = getMediaArchive().query("hotfolder").all().sort("orderingDown").sort("lastscanstart")
					.search();
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.entermediadb.asset.Bob#setMediaArchive(org.entermediadb.asset.
	 * MediaArchive)
	 */
	...
	public HitTracker saveFilesAndImport(final String currentcollection, final boolean createCategories,
			final Map metadata, final Map pages, final User user) {
	 ...
	 * {
	 * for (Iterator iterator = loadFolders(inCatalogId).iterator();
	 * iterator.hasNext();)
	 * {
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.entermediadb.asset.scanner.HotFolderManager2#deleteFolder(java.lang.
	 * String, org.openedit.Data)
	 ...
	 * 
	 * @see
	 * org.entermediadb.asset.scanner.HotFolderManager2#saveFolder(java.lang.String,
	 * org.openedit.Data)
	 */
	public void saveSourceConfig(Data inNewrow) {
		 ...
		 * Page toplevel = getPageManager().getPage("/WEB-INF/data/" + inCatalogId +
		 * "/hotfolders/" + toplevelfolder +"/" );
		 * if(!toplevel.exists()){
		 ...
		 * (MediaArchive)getSearcherManager().getModuleManager().getBean(inCatalogId,
		 * "mediaArchive");
		 * 
		 * ContentItem hotfolderpath = archive.getContent( "/WEB-INF/data/" +
		 * archive.getCatalogId() + "/workingfolders/"+ email + "/" + toplevelfolder );
		 * File file = new File( hotfolderpath.getAbsolutePath() );
		 * file.mkdirs();
		 * inNewrow.setProperty("externalpath",file.getAbsolutePath());
		 * getFolderSearcher(inCatalogId).saveData(inNewrow, null);
		 * archive.fireMediaEvent("hotfolder", "googledrivesaved", inNewrow.getId(),
		 * null);
		 * 
		 * }
		 ...
		 * 
		 * ContentItem hotfolderpath = archive.getContent( "/WEB-INF/data/" +
		 * archive.getCatalogId() + "/workingfolders/"+ toplevelfolder );
		 * File file = new File( hotfolderpath.getAbsolutePath() );
		 * file.mkdirs();
		 * inNewrow.setProperty("externalpath",file.getAbsolutePath());
		 * getFolderSearcher(inCatalogId).saveData(inNewrow, null);
		 * archive.fireMediaEvent("hotfolder","resiliodrivesaved", inNewrow.getId(),
		 * null);
		 * }
		 * else
		 * {
	...
	public void scanSources(ScriptLogger inLog) {
		...
		for (Iterator iterator = sources.iterator(); iterator.hasNext();) {
			...
			if (source.getConfig() != null) {
				...
				if (periodString != null) {
					...
					Date laststarted = DateStorageUtil.getStorageUtil()
							.parseFromObject(source.getConfig().getValue("lastscanstart"));
					if (laststarted != null) {
			...
			try {
				...
				inLog.info("Hot folder: " + name + ", imported " + found + " assets within:" + timetook / 1000D
						+ " seconds");
			} catch (Exception ex) {
