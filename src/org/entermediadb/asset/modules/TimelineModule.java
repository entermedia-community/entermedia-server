...
public class TimelineModule extends BaseMediaModule {
	...
	public MultiValued loadClosedCaption(WebPageRequest inReq) {
		...
		MultiValued track = (MultiValued) captionsearcher.query().exact("assetid", asset.getId())
				.exact("sourcelang", lang).sort("timecodestart").searchOne();
	...
	public Collection loadClosedCaptions(WebPageRequest inReq) {
		...
		Asset asset = getAsset(inReq);
		if (asset != null) {
			Collection tracks = captionsearcher.query().exact("assetid", asset.getId()).sort("sourcelang")
					.search(inReq);
	...
	public void importCaptions(WebPageRequest inReq) throws Exception {
		...
		// Remove old ones
		HitTracker existing = searcher.query().exact("assetid", asset.getId()).exact("sourcelang", selectedlang)
				.search(); // TODO per lang
	...
	public void addCaption(WebPageRequest inReq) {
		...
		Data lasttrack = captionsearcher.query().exact("assetid", asset.getId()).exact("sourcelang", selectedlang)
				.searchOne();
		if (lasttrack == null) {
	...
	protected void saveCaptionToFullText(MediaArchive inArchive, Asset inAsset) {
		...
		ContentItem item = getPageManager().getRepository().getStub(
				"/WEB-INF/data/" + inArchive.getCatalogId() + "/assets/" + inAsset.getSourcePath() + "/fulltext.txt");
	...
	public void loadCaptionEditor(WebPageRequest inReq) {
		...
		MultiValued track = (MultiValued) captionsearcher.query().exact("assetid", asset.getId())
				.exact("sourcelang", selectedlang).sort("timecodestart").searchOne();
		track = (MultiValued) captionsearcher.loadData(track);
		inReq.putPageValue("track", track);
		inReq.putPageValue("captionsearcher", captionsearcher);
	...
	public void addAutoTranscode(WebPageRequest inReq) throws Exception {
		...
		// runpath="/${catalogid}/events/conversions/autotranscode.html"
		// allowduplicates="true" />
		MediaArchive archive = getMediaArchive(inReq);

		GoogleTranscriberManager manager = (GoogleTranscriberManager) getModuleManager().getBean(archive.getCatalogId(),
				"googleTranscriberManager");
		...
		// inRe
		// GoogleTranscriberManager manager =
		// (GoogleTranscriberManager)getModuleManager().getBean(catalogid,
		// "googleTranscriberManager");
	...
	public void autoTranscodeQueue(WebPageRequest inReq) throws Exception {
		...
		String selectedlang = inReq.getRequestParameter("selectedlang");
		Collection hits = captionsearcher.query().or().exact("transcribestatus", "inprogress")
				.exact("transcribestatus", "needstranscribe").search();
		if (!hits.isEmpty()) {
			log.info("Transcribing: " + hits.size());
		}
		for (Iterator iterator = hits.iterator(); iterator.hasNext();) {
			Data track = (Data) iterator.next();
			Lock lock = archive.getLockManager().lockIfPossible("transcode" + track.get("assetid"),
					"autoTranscodeQueue");
			try {
				if (lock != null) {
	...
	public void addAutoTranslate(WebPageRequest inReq) throws Exception {
		...
		Data newtrack = captionsearcher.query().exact("assetid", asset.getId()).exact("sourcelang", targetlang)
				.searchOne();
		if (newtrack == null) {
	...
	protected Collection<Map> translateInGroups(Searcher captionsearcher, String selectedlang, String targetlang,
			Collection<Map> existingcaptions) throws InterruptedException {
