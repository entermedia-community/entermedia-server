...
public class AssetEditModule extends BaseMediaModule {
	...
	/*
	 * public void saveAssetProperties(WebPageRequest inReq) throws
	 * OpenEditException
	 * {
	...
	public void createAssetsFromFile(WebPageRequest inReq) {
		...
		Asset asset = getAssetImporter().createAssetFromExistingFile(getMediaArchive(catalogid), inReq.getUser(),
				sourcepath);
		...
		for (Iterator iterator = properties.getUploadItems().iterator(); iterator.hasNext();) {
			...
			if (target != null) {
				...
				if (detail != null) {
					...
					// //Legacy work around
					// if( sourcemask != null && sourcemask.contains("uploadsourcepath") &&
					// target.get("sourcepath") == null)
					// {
					...
					if (sourcemask == null) {
						if (searcher.getDetail("sourcepath") != null) {
							...
							Category cat = archive.getEntityManager().loadDefaultFolder(entitmodule, target,
									inReq.getUser());
							if (cat != null) {
					...
					if (sourcepath == null && sourcemask != null) {
						// String sourcepath = inReq.getRequestParameter(detailid + ".sourcepath"); //Is
						// this risky?
						...
						sourcepath = getAssetImporter().getAssetUtilities().createSourcePathFromMask(archive, null,
								inReq.getUser(), item.getName(), sourcemask, variables);
			...
			current = getAssetImporter().getAssetUtilities().populateAsset(null, item.getSavedPage().getContentItem(),
			...
			archive.getAssetEditor().createNewVersionData(current, originalfile.getContentItem(), inReq.getUserName(),
					Version.UPLOADED, null);
	...
	public void rotateAsset(WebPageRequest inReq) {
		...
		String custompath = "/WEB-INF/data/" + archive.getCatalogId() + "/generated/" + asset.getSourcePath()
				+ "/customthumb.jpg";
