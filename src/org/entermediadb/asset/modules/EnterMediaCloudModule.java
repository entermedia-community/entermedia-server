...
public class EnterMediaCloudModule extends BaseJsonModule {
	...
	public void loginUserWithExpiringKey(WebPageRequest inReq) {
		...
		if ("ok".equals(status)) {
			...
			BaseAutoLogin autologin = (BaseAutoLogin) getModuleManager().getBean(archive.getCatalogId(),
					"autoLoginWithCookie");
	...
	protected void downloadAssets(WebPageRequest inReq, MediaArchive archive, Category category, List tosave) {
		...
		Page savefolder = getPageManager()
				.getPage("/WEB-INF/data/" + archive.getCatalogId() + "/originals/" + savepath);
		...
		for (Iterator iterator = tosave.iterator(); iterator.hasNext();) {
			...
			String download = getEndpointurl() + "/services/module/asset/downloads/originals/"
					+ archive.asLinkToOriginal(newasset);
