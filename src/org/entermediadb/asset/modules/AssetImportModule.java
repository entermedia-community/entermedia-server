...
public class AssetImportModule extends BaseMediaModule {
	 ...
	 * Data entity = archive.getCachedData( moduleid
	 * ,(String)params.get("entityid"));
	 * 
	 * String categorypath = (String)params.get("categorypath");
	 * Category category =
	 * archive.getCategorySearcher().loadCategoryByPath(categorypath);
	 * //EntityManager entityManager = getEntityManager(inReq);
	 * //Category category = entityManager.loadDefaultFolder(module, entity,
	 * inReq.getUser());
	 * 
	 * FolderManager manager = getFolderManager(inReq);
	...
	public void exportCollection(WebPageRequest inReq) {
		 ...
		 * // delete.
		 * // String infolder = "/WEB-INF/data/" + archive.getCatalogId() +
		 * "/workingfolders/" + user.getId() + "/"
		 * // + collection.getName();
		 * //
		 * // Date now = new Date();
		 * //
		 * // String stamp = DateStorageUtil.getStorageUtil().formatDateObj(now,
		 * "yyyy-MM-dd-HH-mm-ss");
		 * //
		 ...
		 * 
		 * boolean lock =
		 * Boolean.parseBoolean(inReq.getRequestParameter("lockcollection"));
		 * if(lock) {
