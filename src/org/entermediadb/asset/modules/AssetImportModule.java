...
public class AssetImportModule extends BaseMediaModule
{
	...
	public void importAssets(WebPageRequest inReq)
	{
		...
		// mediaArchive.isCatalogSettingTrue("assigncategoryonupload");
		// if(assigncategory) {
		// hits.each{
		// Asset current = it;
		// Category defaultcat =
		// mediaArchive.getCategorySearcher().createCategoryPath(current.sourcePath);
		//
		...
		/*
		 * LibraryManager librarymanager = new LibraryManager(); librarymanager.setLog(logger);
	...
	public void desktopOpenFolder(WebPageRequest inReq)
	{
		...
		// Desktop desktop =
		// manager.getDesktopManager().getDesktop(inReq.getUserName());
	...
	public void sendDesktopCommand(WebPageRequest inReq)
	{
		try
		{
			...
			/*
			 * Desktop desktop = manager.getDesktopManager().getDesktop(inReq.getUserName()); JSONParser parser
			 * = new JSONParser(); JSONObject command = (JSONObject) parser.parse(commands);
			 * 
	...
	public void downloadCollectionToClient(WebPageRequest inReq, MediaArchive inMediaArchive, String inCollectionid)
	{
		...
		/*
		 * Desktop desktop = manager.getDesktopManager().getDesktop(inReq.getUserName()); if (desktop ==
		 * null) { throw new OpenEditException("Desktop disconnected"); }
		 */
	...
	public void checkPullSummary(WebPageRequest inReq)
	{
		...
		// Category category =
		// archive.getCategorySearcher().loadCategoryByPath(categorypath);
		//
		// HitTracker hits =
		// archive.getAssetSearcher().query().named("serverassets").exact("category",
		// category.getId()).search(inReq);
		//
	...
	/*
	 * 
	 * public void checkPushRemoteFolder(WebPageRequest inReq) { Map params = inReq.getJsonRequest(); if
	 * (params == null) { return; } inReq.putPageValue("remotemap", new JSONObject(params));
	 * 
	 * MediaArchive archive = getMediaArchive(inReq); String moduleid = (String)params.get("moduleid");
	 * Data module = archive.getCachedData("module", moduleid); Data entity = archive.getCachedData(
	 * moduleid ,(String)params.get("entityid"));
	 * 
	 * String categorypath = (String)params.get("categorypath"); Category category =
	 * archive.getCategorySearcher().loadCategoryByPath(categorypath); //EntityManager entityManager =
	 * getEntityManager(inReq); //Category category = entityManager.loadDefaultFolder(module, entity,
	 * inReq.getUser());
	 * 
	 * FolderManager manager = getFolderManager(inReq); Map assetmap = manager.listAssetMap(archive,
	 * category);
	 * 
	 * //List remoteassets = (List)params.get("files"); Map pendinguploads =
	 * manager.removeDuplicateAssetsPush(assetmap,params); inReq.putPageValue("assetmap", new
	 * JSONObject(pendinguploads));
	 * 
	 * //Removed files locally Map missingassets = manager.findMissingAssetsFromPush(assetmap, params);
	...
	{
		/*
		 * FolderManager manager = getFolderManager(inReq); Desktop desktop =
		 * manager.getDesktopManager().getDesktop(inReq.getUser()); inReq.putPageValue("desktop",desktop);
		 */
		return;
	}
	...
	public void exportCollection(WebPageRequest inReq)
	{
		...
		/*
		 * Desktop desktop = manager.getDesktopManager().getDesktop(inReq.getUserName()); if(
		 * desktop.isBusy()) { log.info("Desktop still busy"); return; }
		 */
		/*
		 * String collectionid = loadCollectionId(inReq);
		 * 
		 * User user = inReq.getUser(); if (user == null) { throw new OpenEditException("User required "); }
		 * // Data collection = archive.getData("librarycollection", collectionid);
		 * 
		 * // The trailing slash is needed for the recursive algorithm. Don't // delete. // String infolder
		 * = "/WEB-INF/data/" + archive.getCatalogId() + "/workingfolders/" + user.getId() + "/" // +
		 * collection.getName(); // // Date now = new Date(); // // String stamp =
		 * DateStorageUtil.getStorageUtil().formatDateObj(now, "yyyy-MM-dd-HH-mm-ss"); // // infolder =
		 * infolder + "/" + stamp + "/";
		 * 
		 * manager.downloadCollectionToClient(inReq, archive, collectionid);
		 * 
		 * 
		 * boolean lock = Boolean.parseBoolean(inReq.getRequestParameter("lockcollection")); if(lock) {
