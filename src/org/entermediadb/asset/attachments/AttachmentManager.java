...
public class AttachmentManager {
	...
	public void processAttachments(MediaArchive inArchive, Asset inAsset, boolean reprocess) {
		...
		syncFolder(attachmentSearcher, inArchive, inAsset.getId(), root, inAsset.getSourcePath(),
				inAsset.getSourcePath(), reprocess);
	}
	...
	protected void syncFolder(Searcher attachmentSearcher, MediaArchive inArchive, String inAssetId,
			String inRootFolder, String inFolderSourcePath, String inAssetSourcePath, boolean inReprocess) {
		SearchQuery query = attachmentSearcher.createSearchQuery();
		...
		if (inReprocess || hits.size() == 0) // Could not find any existing children, add a child
		{
			...
			for (Iterator iterator = files.iterator(); iterator.hasNext();) {
				...
				if (page.isFolder()) {
					syncFolder(attachmentSearcher, inArchive, inAssetId, inRootFolder,
							inFolderSourcePath + "/" + page.getName(), inAssetSourcePath, inReprocess);
				}
			}
			// delete old records
	...
	public void createFolder(WebPageRequest inReq, MediaArchive inArchive, Asset inAsset, String inParentid,
			String inName) {
		Searcher attachmentSearcher = getAttachmentSearcher(inArchive.getCatalogId());
	...
	public void renameFolder(WebPageRequest inReq, MediaArchive inArchive, Asset inAsset, String inFolderId,
			String inNewName) {
		Searcher attachmentSearcher = getAttachmentSearcher(inArchive.getCatalogId());
		String sourcepath = null;
		if (inFolderId == null) {
