...
public class vizonepublisher extends BasePublisher implements Publisher {
	...
	// vpm.restrictions should contain the values of UserRestrictions and
	// UserRestInstruction seperated by ','
	//
	...
	public void updateAsset(MediaArchive inArchive, String servername, Asset inAsset, String inAuthString)
		...
		// "https://vmeserver/thirdparty/asset/item?format=opensearch"

		String addr = servername + "api/asset/item/" + inAsset.get("vizid") + "/metadata";

		// Change URL -
		// String data = "<payload xmlns='http://www.vizrt.com/types'
		// model=\"http://vizmtlvamf.media.in.cbcsrc.ca/api/metadata/form/vpm-item/r1\"><field
		// name='asset.title'><value>${title}</value></field> <field
		// name='asset.owner'><value>Img</value></field> <field
		// name='asset.retentionPolicy'> <value>${policy}</value> </field> </payload>";
	...
	private String getRestrictionValue(Asset inAsset) {
		...
		// log.info("*** getRestrictionValue: "+rest_val +" , "+right_val +" ,
		// "+term_val);
	...
	private boolean updateAclElement(Element elemAcl, String elementName, String[] rra) {
		...
		if (!externalAppElementFound) {
			...
			// log.info("***updateAclElement: read: "+rra[0] +" write: "+rra[1] +" admin: "
			// +rra[2] +" for "+elementName);
