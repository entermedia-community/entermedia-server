...
public class CategoryEditor
{
	...
	public void saveCategory(Category inCategory, Boolean saveTree) throws OpenEditRuntimeException
	{
		...
		// try
		// {
		// Page desc = getPageManager().getPage(getMediaArchive().getCatalogHome() +
		// "/categories/" + inCategory.getId() + ".html");
		// if ( !desc.exists() )
		// {
		// StringItem item = new StringItem(desc.getPath(), "
		// ",desc.getCharacterEncoding() );
	 ...
	 * String finalpath = inFolder + "/" + inRoot.getName(); Searcher assets =
	 * inArchive.getAssetSearcher(); Searcher cats = inArchive.getSearcher("category");
	 * 
	 * HitTracker assetlist = assets.fieldSearch("category-exact", inRoot.getId()); for (Iterator
	 * iterator = assetlist.iterator(); iterator.hasNext();) { Data hit = (Data) iterator.next(); Asset
	 * asset = (Asset) assets.loadData(hit); Page fullpath =
	 * inArchive.getPageManager().getPage("/WEB-INF/data/" + inArchive.getCatalogId() + "/originals/" +
	 * asset.getPath()); log.info(fullpath.isFolder()); if (!fullpath.exists()) { log.info("Fullpath " +
	 * fullpath + "Did not exist");
	 * 
	 * } log.info(fullpath); Page target = inArchive.getPageManager().getPage(finalpath);
	 * inArchive.getPageManager().copyPage(fullpath, target);
	 * 
	 * }
	 * 
	 * for (Iterator iterator = inRoot.getChildren().iterator(); iterator.hasNext();) { Category child =
	 * (Category) iterator.next(); exportCategoryTree(inArchive, child, finalpath); }
	 * 
	 * }
	 */
	...
	public void copyTree(Category fromchild, Category target)
	{
		...
		// <path-action name="PathEventModule.runEvent"
		// runpath="/${catalogid}/events/categories/assetsadded.html"
	...
	// {
	// Page totrash = getPageManager().getPage("/WEB-INF/data/" +
	// getMediaArchive().getCatalogId() + "/categories.xml" );
	// getPageManager().removePage(totrash);
	//
	// String datadir = "/WEB-INF/data/" + getMediaArchive().getCatalogId() +
	// "/originals/";
	// getMediaArchive().getCategoryArchive().clearCategories();
	// //Category root =
	// getMediaArchive().getCategoryArchive().createNewCategory("Index");
	// //getMediaArchive().getCategoryArchive().setRootCategory(root);
	//
	// reBuildCategories(getMediaArchive().getCategoryArchive(), root,
	// datadir,datadir);
	//
	// }
	//
	// private void reBuildCategories(CategoryArchive inCategoryArchive, Category
	// inParent, String inStartingFrom, String inFolder )
	// {
