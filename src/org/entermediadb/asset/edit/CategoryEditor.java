...
public class CategoryEditor {
	...
	public void moveCategoryBefore(Category inCategory, Category inBeforeCategory) throws OpenEditRuntimeException {
		...
		if (inBeforeCategory == null || inBeforeCategory.getParentCategory() != parent
				|| inCategory == inBeforeCategory)
			return;
	...
	public void saveCategory(Category inCategory, Boolean saveTree) throws OpenEditRuntimeException {
		if (inCategory.getParentCategory() == null
				&& getMediaArchive().getCategoryArchive().getRootCategory().getId() != inCategory.getId()) {
	...
	public void changeCategoryId(Category inCategory, String inId) throws OpenEditException {
		...
		// reload = true;
		Page oldPage = pageManager
				.getPage(getMediaArchive().getCatalogHome() + "/categories/" + inCategory.getId() + ".html");
	...
	public void addAssetsToCategory(String[] inAssetIds, String inPrefix, String inSuffix, Category inCategory)
		...
		if (inCategory != null) {
			...
			for (Iterator iter = assets.iterator(); iter.hasNext();) {
				...
				if (inPrefix != null || inSuffix != null) {
					...
					asset = getMediaArchive().getAssetEditor().copyAsset(element,
							inPrefix + element.getId() + inSuffix);
				}
				if (asset != null) {
	...
	public void moveAssetsToCategory(String[] inAssetid, Category inCategory1, Category inCategory2)
			throws OpenEditRuntimeException {
		List assetsToSave = new ArrayList();
	...
	/*
	 * public void copyCategory(MediaArchive inArchive, Category inSource, Category
	 * inDestination)
	 * {
	 ...
	 * Page fullpath = inArchive.getPageManager().getPage("/WEB-INF/data/" +
