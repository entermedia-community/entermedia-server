...
public class AssetEditor {
	...
	public void deleteAsset(Asset inAsset, User inUser) throws OpenEditRuntimeException {
		...
		// inAsset.getId().equals(getCurrentAsset().getId()))
		// {
	...
	public void deleteAsset(Asset asset, HitTracker inTracker, boolean inDeleteOriginal, User inUser) {
		...
		log.info("Asset Deleted - assetid " + asset.getId() + " - user " + inUser + " - sourcepath: "
				+ asset.getSourcePath() + " original: " + inDeleteOriginal);
	}
	...
	public boolean makeFolderAsset(Asset inAsset, User inUser) {
		...
		carchive.saveComments("/WEB-INF/data/" + getMediaArchive().getCatalogId() + "/comments/" + newSourcePath,
				allcomments);
	...
	public Asset copyAsset(MediaArchive inArchive, Asset inAsset, String inDestSourcePath) {
		...
		String oldSourcePath = inAsset.getSourcePath();
		String oldpath = "/WEB-INF/data/" + inArchive.getCatalogId() + "/originals/" + oldSourcePath; // If its a folder
																										// if will
																										// include
																										// children
		...
		Page oldthumbs = inArchive.getPageManager()
				.getPage("/WEB-INF/data/" + inArchive.getCatalogId() + "/generated/" + inAsset.getSourcePath());
		if (!inDestSourcePath.endsWith("/")) {
			inDestSourcePath = inDestSourcePath + "/";
		}
		Page newthumbs = inArchive.getPageManager()
				.getPage("/WEB-INF/data/" + inArchive.getCatalogId() + "/generated/" + inDestSourcePath);
		if (oldthumbs.exists()) {
	...
	public void fullyRemoveAsset(Asset inAsset, User inUser, boolean inKeepReleases) {
		...
		if (!inKeepReleases) {
			Page releases = getPageManager()
					.getPage("/WEB-INF/data/" + inAsset.getCatalogId() + "/releases/" + inAsset.getSourcePath());
			if (releases.exists()) {
				getPageManager().removePage(releases);
			}
		}
		// remove the originals folder
		Page originals = getPageManager()
				.getPage("/WEB-INF/data/" + inAsset.getCatalogId() + "/originals/" + inAsset.getSourcePath());
		if (originals.exists()) {
		...
		Page data = getPageManager()
				.getPage("/WEB-INF/data/" + inAsset.getCatalogId() + "/assets/" + inAsset.getSourcePath());
		if (data.exists()) {
	...
	public Element createNewVersionData(Asset inCurrent, ContentItem inTosavePath, String inUserName,
			String inChangeType, String inUserMessage) {
	...
	public void backUpFilesForLastVersion(Asset inCurrent, ContentItem inPreviousFile, ContentItem inPreviewImage) {
		...
		String originalcopy = PathUtilities.extractDirectoryPath(inXmlFile.getPath()) + "/" + versionnum + "~"
				+ inPreviousFile.getName();
		...
		if (inPreviewImage != null && inPreviewImage.exists()) {
			...
			String previewcopypath = PathUtilities.extractDirectoryPath(inXmlFile.getPath()) + "/" + versionnum + "~"
					+ inPreviousFile.getName() + "preview." + type;
	...
	public void replaceOrginal(Asset current, ContentItem inNewContent, String inEditedBy, String inMessage) {
		...
		ContentItem preview = getMediaArchive().getPresetManager().outPutForGenerated(archive, current,
				"image3000x3000");
		...
		getMediaArchive().getAssetImporter().getAssetUtilities().getMetaDataReader().populateAsset(archive,
				inNewContent, current);
	...
	public void restoreVersion(Asset current, String inEditUser, String inVersion) {
		...
		ContentItem preview = getMediaArchive().getPresetManager().outPutForGenerated(getMediaArchive(), current,
				"image3000x3000");
		backUpFilesForLastVersion(current, original, preview);
		...
		// Version the new one
		getMediaArchive().getAssetImporter().getAssetUtilities().getMetaDataReader().populateAsset(getMediaArchive(),
				original, current);
	...
	public void reloadThumbnails(Asset inAsset) {
		...
		MediaArchive archive = getMediaArchive();
		HitTracker conversions = archive.query("conversiontask").exact("assetid", inAsset.getId()).search(); // This is
																												// slow,
																												// we
																												// should
																												// load
		...
		// Good idea?
		// archive.fireMediaEvent("conversions","runconversion", inReq.getUser(),
		// inAsset);//block?
	...
	protected Version createRevision(String inPath, Element element) {
		// <version number="1" author="admin" date="2323525233" type="added">Some
		// message</version>
