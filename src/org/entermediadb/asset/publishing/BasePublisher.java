...
public abstract class BasePublisher implements Publisher {
	...
	protected PublishResult checkOnConversion(MediaArchive mediaArchive, Data inOrderItem, Asset inAsset,
			Data inPreset) {
		PublishResult result = new PublishResult();
		...
		if ("0".equals(inPreset.getId()))// || isremote)
		{
			...
			if ("error".equals(cstatus)) // missinginput is ok
			{
				result.setErrorMessage(
						inPreset.getName() + " conversion had an error: " + conversiontask.get("errordetails"));
				return result;
