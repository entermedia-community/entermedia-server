...
public class BaseSearchSecurity implements SearchSecurity {
	...
	public SearchQuery attachSecurity(WebPageRequest inPageRequest, Searcher inSearcher, SearchQuery inQuery) {
		SearchSecurity filter = (SearchSecurity) getCacheManager().get("searchfilters",
				inSearcher.getCatalogId() + inSearcher.getSearchType());
		if (filter == null) {
			// Look up in the database a cache for this filter type
			Data config = getSearcherManager().getData(inSearcher.getCatalogId(), "searchsecurity",
					inSearcher.getSearchType());
			if (config != null) {
				String beanname = config.get("beanname");

				// Legacy fix
				beanname = beanname.replace("QueryFilter", "Security");
				filter = (SearchSecurity) getSearcherManager().getModuleManager().getBean(inSearcher.getCatalogId(),
						beanname);
			} else {
				if (getSearcherManager().getModuleManager()
						.doesBeanExist(inSearcher.getSearchType() + "SearchSecurity")) {
					filter = (SearchSecurity) getSearcherManager().getModuleManager().getBean(inSearcher.getCatalogId(),
							inSearcher.getSearchType() + "SearchSecurity");
				} else {
					PropertyDetail detail = inSearcher.getDetail("securityenabled");
					if (detail != null) {
						filter = (SearchSecurity) getSearcherManager().getModuleManager()
								.getBean(inSearcher.getCatalogId(), "securityEnabledSearchSecurity");
					} else {
						filter = NULL;
					}
				}
	...
	public SearchQuery attachStandardSecurity(WebPageRequest inPageRequest, Searcher inSearcher, SearchQuery inQuery) {
		...
		QueryBuilder builder = inSearcher.query().or().orgroup("viewgroups", groupids).exact("viewroles", roleid)
				.exact("owner", userid).exact("viewusers", userid);
