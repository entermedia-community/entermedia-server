...
public class PresetCreator {
	...
	public Collection getPresets(MediaArchive inArchive, String rendertype) {
		...
		if (hits == null) {
			Searcher presetsearcher = inArchive.getSearcher("convertpreset");
			hits = presetsearcher.query().exact("onimport", "true").exact("inputtype", rendertype).sort("ordering")
					.search();
	...
	public Collection queueConversions(MediaArchive mediaarchive, Searcher tasksearcher, Data asset,
			boolean forcererun) {
		...
		// asset.get("fileformat").startsWith("gd"))
		// {
		...
		HitTracker conversions = tasksearcher.query().exact("assetid", asset.getId()).orgroup("presetid", presets)
				.search(); // This is slow, we should load up a bunch at once
	...
	public int createMissing(MediaArchive mediaarchive, Searcher tasksearcher, Map existingtasks, List tosave,
			Data preset, Data asset) {
		int added = 0;
		boolean missingconversion = false;
	...
	public void checkAssetConversions(MediaArchive inArchive, Data asset, Collection assetconversions) {
		...
		if (founderror || allcomplete) {
			...
			if (asset != null) {
				 ...
				 * if(founderror && "error".equals(existingimportstatus) ||
				 * ("complete".equals(existingimportstatus) &&
				 * "2".equals(existingpreviewstatus)))
				 * {
				...
				inArchive.fireSharedMediaEvent("importing/importcomplete"); // TODO: add importscomplete for when we
																			// have saved a number of items
			}
		}
	}

	public Data getPresetByOutputName(MediaArchive inArchive, String inRenderType, String inFileName) {
		return (Data) inArchive.getSearcher("convertpreset").query().exact("generatedoutputfile", inFileName)
				.exact("inputtype", inRenderType).searchOne();
	}
	...
	public void conversionCompleted(MediaArchive inArchive, Asset inAsset) {
		...
		HitTracker assetconversions = tasksearcher.query().exact("assetid", inAsset.getId()).search(); // This is slow,
																										// we should
																										// load up a
																										// bunch at once
		...
		// HitTracker conversions = tasksearcher.query().match("assetid",
		// asset.getId()).search();
	...
	public ContentItem outPutForPreset(MediaArchive inArchive, Asset inAsset, Data inPreset) {
		if ("0".equals(inPreset.getId())) {
			...
			String generatedfilename = "/WEB-INF/data/" + inArchive.getCatalogId() + "/generated/"
					+ inAsset.getSourcePath() + "/" + inPreset.get("generatedoutputfile");
			ContentItem output = inArchive.getContent(generatedfilename);
			return output;
		}
		return null;
