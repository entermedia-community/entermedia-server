...
public abstract class BaseTranscoder implements MediaTranscoder {
	...
	/**
	 * Instructions require a completely pre-dermined output page in case we already
	 * coverted it
	 * Previously
	 */
	public ConvertResult createOutput(ConvertInstructions inStructions) {
		...
		if (asset == null) {
			asset = inStructions.getMediaArchive().getAssetImporter().createAsset(inStructions.getMediaArchive(),
					sourcePath); // virtual assets
		}
		if (asset == null) {
			asset = inStructions.getMediaArchive().getAssetImporter().createAsset(inStructions.getMediaArchive(),
					inStructions.getInputPath());
		}
		if (asset == null) {
			return null;
		}
	...
	/**
	 * public ConvertInstructions createInstructions(Page inDef, MediaArchive
	 * inArchive, String inOutputType, String inSourcePath)
	 * {
	 * Map all = new HashMap();
	 * for (Iterator iterator =
	 * inDef.getPageSettings().getAllProperties().iterator(); iterator.hasNext();)
	 * {
