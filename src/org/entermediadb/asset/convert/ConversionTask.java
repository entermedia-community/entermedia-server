...
public class ConversionTask {
	...
	public void convert() {
		...
		if (result.isOk()) {
			if (result.isComplete()) {
				...
				// {
				// item.setProperty("publishstatus", "readytopublish"); ////new readytopublish
				// publishing complete
	...
	protected ConvertResult doConversion(MediaArchive inArchive, Data inTask, Data inPreset, Asset inAsset) {
		...
		// log.debug("Converting with type: ${type} using ${creator.class} with status:
		// ${status}");
		...
		if (guid != null) {
			Searcher presetdatasearcher = inArchive.getSearcherManager().getSearcher(inArchive.getCatalogId(),
					"presetdata");
		...
		if ("new".equals(status) || "retry".equals(status) || "missinginput".equals(status)) {
			// String outputpage =
			// "/WEB-INF/data/${inArchive.catalogId}/generated/${asset.sourcepath}/${inPreset.outputfile}";
			// String outputpage = creator.populateOutputPath(inArchive, inStructions,
			// inPreset);
			...
			tmpresult = new ConvertResult();
			tmpresult.setOk(false);
			tmpresult.setError(inTask.getId() + " task id with status:" + status
					+ " Should have been: submitted, new, missinginput or retry, is index out of date? ");
		}
