...
public class VideoConversionManager extends BaseConversionManager {
	...
	public ConvertResult transcode(ConvertInstructions inStructions) {
		...
		Data preset = getMediaArchive().getPresetManager().getPresetByOutputName(inStructions.getMediaArchive(),
				"video", "video.m3u8");
		if (preset == null || !Boolean.parseBoolean(preset.get("onimport"))) {
			// legacy
			preset = getMediaArchive().getPresetManager().getPresetByOutputName(inStructions.getMediaArchive(), "video",
					"video.mp4");
		}
		...
		Data imagepreviewpreset = getMediaArchive().getPresetManager()
				.getPresetByOutputName(inStructions.getMediaArchive(), "video", defaultpresetoutput);
		if (imagepreviewpreset == null) {
			defaultpresetoutput = "image1900x1080.jpg";
			imagepreviewpreset = getMediaArchive().getPresetManager()
					.getPresetByOutputName(inStructions.getMediaArchive(), "video", defaultpresetoutput);
		}
		if (imagepreviewpreset == null) {
		...
		// Finally use ImageMagick to transform the final image using the image above as
		// the input
		// preset =
