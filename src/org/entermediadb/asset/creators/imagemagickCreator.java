package org.entermediadb.asset.creators;
...
import org.entermediadb.asset.creator.BaseCreator;
import org.entermediadb.asset.creator.ConvertInstructions;
import org.entermediadb.asset.creator.ConvertResult;
import org.entermediadb.asset.creator.ConvertToImageInstructions;
...
public class imagemagickCreator extends BaseCreator
{
	...
	{ 
		return new ConvertToImageInstructions(inArchive);
	}
	...
	public ConvertResult convert(ConvertInstructions inStructions) throws OpenEditException
	{
		ConvertResult result = new ConvertResult();
		
		Page input = createInput(inStructions);
	 ...
	 * inArchive.getOriginalFileManager().getFilePath(inStructions.getSourceFile
	 * (), asset); } else { original =
	 ...
	 * inStructions.setInputType(extension); String orientation =
	 * asset.get("imageorientation"); if (orientation != null) { Data ed =
	 * (Data) inArchive.getSearcherManager().getSearcher(asset.getCatalogId(),
	 * "imageorientation").searchById(orientation); if (ed != null) { String
	 * rotation = ed.get("rotation"); if (rotation != null &&
	 * !rotation.equals("")) {
	 * inStructions.setRotation(Integer.parseInt(rotation)); } } } File input =
	 * new File(original); return input; }
	 */
	 ...
	 * new File(stub.getAbsolutePath()); if (watermarked.exists() &&
	 * watermarked.length() > 0) { return watermarked; } else {
	 * watermarked.getParentFile().mkdirs(); try { watermarked.createNewFile();
	 * } catch (IOException e) { throw new OpenEditException(e); } // watermark
	 * it MediaCreator convert =
