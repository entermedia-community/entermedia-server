...
public class Parser
{
     ...
     * </pre>
     * 
     * Note that if you have a file with a byte order mark (BOM), you need to skip it before handing the
     * reader to this library. There's existing classes that do that, e.g. see {@code BOMInputStream} in
     * Commons IO.
     * <p>
     * This method is thread-safe (a new parser state is used for each invocation).
     *
    ...
    {
        return new DocumentParser(blockParserFactories, inlineParserFactory, inlineContentParserFactories, delimiterProcessors, linkProcessors, linkMarkers, includeSourceSpans);
    }
    ...
    public static class Builder
    {
         ...
         * </pre>
         *
         * @param enabledBlockTypes A list of block nodes the parser will parse. If this list is empty, the
         *        parser will not recognize any CommonMark core features.
         * @return {@code this}
         */
         ...
         * extensions can change how some syntax is parsed that would otherwise be handled by built-in
         * factories. "With great power comes great responsibility."
         *
         ...
         * {@link InlineContentParserFactory#getTriggerCharacters()}. It is possible to register multiple
         * parsers for the same character, or even for some built-in special character such as {@code `}.
         * The custom parsers are tried first in order in which they are registered, and then the built-in
         * ones.
         */
         ...
         * <p>
         * Note that multiple delimiter processors with the same characters can be added, as long as they
         * have a different minimum length. In that case, the processor with the shortest matching length is
         * used. Adding more than one delimiter processor with the same character and minimum length is
         * invalid.
         * <p>
         ...
         * <p>
         * Multiple link processors can be added, and will be tried in order in which they were added. If no
         * link processor applies, the normal behavior applies. That means these can override built-in link
         * parsing.
         *
        ...
        /**
         * Add a custom link marker for link processing. A link marker is a character like {@code !} which,
         * if it appears before the {@code [} of a link, changes the meaning of the link.
         * <p>
         * If a link marker followed by a valid link is parsed, the
         * {@link org.entermediadb.markdown.parser.beta.LinkInfo} that is passed to {@link LinkProcessor}
         * will have its {@link LinkInfo#marker()} set. A link processor should check the
         * {@link Text#getLiteral()} and then do any processing, and will probably want to use
         * {@link LinkResult#includeMarker()}.
         *
         ...
         * <p>
         * Provide an implementation of InlineParserFactory which provides a custom inline parser to modify
         * how the following are parsed: bold (**) italic (*) strikethrough (~~) backtick quote (`) link
         * ([title](http://)) image (![alt](http://))
         * <p>
         * Note that if this method is not called or the inline parser factory is set to null, then the
