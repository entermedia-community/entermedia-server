...
protected Data getOrderItem(String inOrderItemId) {
	Searcher orderitemsearcher = mediaarchive.getSearcherManager().getSearcher(mediaarchive.getCatalogId(), "orderitem");
	Data orderitem = orderitemsearcher.searchById(inOrderItemId);
	return orderitem;
}
...
protected void checkOrderPublish(Order inOrder) {
	...
	HitTracker items = itemsearcher.fieldSearch("orderid", inOrder.getId());
	...
	if (allpublished) {
		...
		//ordersearcher.saveData(inOrder, context.getUser());
	}
}


WebEvent webevent = context.getPageValue("webevent");
String orderItemId = webevent.get('orderitemid');
Data orderitem = getOrderItem(orderItemId);
Order order = getOrder(orderitem.get('orderid'));

//Searcher ordersearcher = mediaarchive.getSearcherManager().getSearcher(mediaarchive.getCatalogId(), "order");
//ordersearcher
//ordersearcher.search()

checkOrderPublish(order);
