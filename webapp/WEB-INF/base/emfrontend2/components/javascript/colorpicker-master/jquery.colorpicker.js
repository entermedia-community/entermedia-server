...
(function ($) {
	...
	var _colorpicker_index = 0,
		...
		_container_inline =
			'<div class="ui-colorpicker ui-colorpicker-inline"></div>',
	...
	$.colorpicker = new (function () {
		...
		this.parsers = {
			...
			"#HEX": function (color) {
				var m = /^#([a-fA-F0-9]{2})([a-fA-F0-9]{2})([a-fA-F0-9]{2})$/.exec(
					color,
				);
				if (m) {
					return new $.colorpicker.Color(
		...
		this.parts = {
			...
			map: function (inst) {
				...
				_html = function () {
					var html =
						'<div class="ui-colorpicker-map ui-colorpicker-map-' +
						(inst.options.part.map.size || 256) +
				...
				this.repaint = function () {
					...
					switch (inst.mode) {
						case "h":
							...
							$(e).css(
								"background-color",
								inst.color.copy().setHSV(null, 1, 1).toCSS(),
							);
							break;
			...
			bar: function (inst) {
				...
				_html = function () {
					var html =
						'<div class="ui-colorpicker-bar ui-colorpicker-bar-' +
						(inst.options.part.bar.size || 256) +
						'  ui-colorpicker-border">' +
				...
				this.repaint = function () {
					...
					switch (inst.mode) {
						...
						case "s":
							...
							$(e).css(
								"background-color",
								inst.color.copy().setHSV(null, 1, null).toCSS(),
							);
							break;
						...
						case "v":
							...
							$(e).css(
								"background-color",
								inst.color.copy().setHSV(null, null, 1).toCSS(),
							);
							break;
			...
			preview: function (inst) {
				...
				this.repaint = function () {
					$(".ui-colorpicker-preview-initial", e)
						.css(
							"background-color",
							inst.currentColor.set ? inst.currentColor.toCSS() : "",
						)
						.attr(
							"title",
							inst.currentColor.set ? inst.currentColor.toCSS() : "",
					...
					$(".ui-colorpicker-preview-current", e)
						.css("background-color", inst.color.set ? inst.color.toCSS() : "")
						.attr("title", inst.color.set ? inst.color.toCSS() : "");
			...
			lab: function (inst) {
				...
				this.init = function () {
					...
					$(".ui-colorpicker-number", part).bind(
						"change keyup",
						function (event) {
							inst.color.setLAB(
								parseInt(
					...
					$(".ui-colorpicker-number", part).bind(
						...
						function (event) {
							inst.color.setCMYK(
								parseInt(
									$(
										".ui-colorpicker-cmyk-c .ui-colorpicker-number",
										part,
									).val(),
									10,
								) / 100,
								parseInt(
									$(
										".ui-colorpicker-cmyk-m .ui-colorpicker-number",
										part,
									).val(),
									10,
								) / 100,
								parseInt(
									$(
										".ui-colorpicker-cmyk-y .ui-colorpicker-number",
										part,
									).val(),
									10,
								) / 100,
								parseInt(
									$(
										".ui-colorpicker-cmyk-k .ui-colorpicker-number",
										part,
									).val(),
									10,
								) / 100,
			...
			hex: function (inst) {
				...
				this.init = function () {
					...
					$(".ui-colorpicker-hex-input", e).bind("change keyup", function () {
						...
						inst.color = _parseHex($(this).val()).setAlpha(
							inst.color.getAlpha(),
						);
						inst._change();
					});
			...
			swatches: function (inst) {
				var that = this,
					...
					html = function () {
						...
						return (
							'<div class="ui-colorpicker-swatches ui-colorpicker-border" style="width:' +
							inst.options.swatchesWidth +
							'px">' +
							html +
							"</div>"
						);
		...
		this.Color = function () {
			var spaces = {
				...
				_hsl_to_rgb = function (hsl) {
					...
					var_2 =
						hsl.l < 0.5 ? hsl.l * (1 + hsl.s) : hsl.l + hsl.s - hsl.s * hsl.l;
					var_1 = 2 * hsl.l - var_2;
			...
			this.toHex = function () {
				var rgb = this.getRGB();
				return (
					_hexify(rgb.r * 255) + _hexify(rgb.g * 255) + _hexify(rgb.b * 255)
				);
			};
	...
	$.widget("vanderlee.colorpicker", {
		...
		_create: function () {
			...
			if (that.element.is("input") || that.options.inline === false) {
				// Initial color
				that._setColor(
					that.element.is("input") ? that.element.val() : that.options.color,
				);
				that._callback("init");
		...
		_setImageBackground: function () {
			if (this.image && this.options.buttonColorize) {
				this.image.css(
					"background-color",
					this.color.set ? this._formatColor("RGBA", this.color) : "",
				);
			}
		},
		...
		_generateInline: function () {
			...
			$(that.element).html(
				that.options.inlineFrame ? _container_inlineFrame : _container_inline,
			);
		...
		_generatePopup: function () {
			...
			$(document).delegate("html", "touchstart click", function (event) {
				...
				for (p = 0; p <= parents.length; ++p) {
					...
					// showOn alt
					if (
						/\balt|both\b/.test(that.options.showOn) &&
						$(that.options.altField).is(parents[p])
					) {
						return;
					}
				}
		...
		_generate: function () {
			...
			that._setColor(
				that.inline || !that.element.is("input")
					? that.options.color
					: that.element.val(),
			);
			...
			if (typeof that.options.parts === "string") {
				if ($.colorpicker.partslists[that.options.parts]) {
					...
					// automatic
					parts_list =
						$.colorpicker.partslists[that.inline ? "inline" : "popup"];
				}
			} else {
		...
		_change: function () {
			...
			if (!this.inline) {
				if (!this.color.set) {
					...
					this.element.val(
						this._formatColor(this.options.colorFormat, this.color),
					);
				}
		...
		_exactName: function (color) {
			...
			this._eachSwatch(function (n, swatch) {
				if (
					color.equals(new $.colorpicker.Color(swatch.r, swatch.g, swatch.b))
				) {
					name = n;
					return false;
				}
		...
		_closestName: function (color) {
			...
			this._eachSwatch(function (n, swatch) {
				d = color.distance(
					new $.colorpicker.Color(swatch.r, swatch.g, swatch.b),
				);
