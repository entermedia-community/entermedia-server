...
updatebasketmediaviewer = function (e) {
	...
	$("#" + targetDiv).load(nextpage, function () {
		...
		$("#mainmedianotifications").html(
			'<div class="alert alert-success alert-save fader">' +
				alerttxt +
				"</div>",
		);
	});
		...
		lQuery(".categorydraggable").livequery(function () {
			$(this).draggable({
				...
				helper: function () {
					...
					$(cloned).css({ border: "1px solid blue", background: "#c9e8f2" });
...
$(document).ready(function () {
	...
	$(document).ajaxError(function (e, jqxhr, settings, exception) {
		...
		if (errordiv.length > 0) {
			...
			$("#errordiv").on("mouseover", function () {
				$(this).stop(true, true).show().css("opacity", 1);
			});

			$("#errordiv p").on("mouseout", function () {
				fade($("#errordiv"));
			});
		} else {
			//  alert("Error \n" + settings.url + " \nreturned " + exception);
