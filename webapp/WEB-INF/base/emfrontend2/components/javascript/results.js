$(document).ready(function (url, params) {
	...
	lQuery("select#selectresultview").livequery(function () {
		...
		select.on("change", function () {
			...
			if (originalhitsperpage) {
				var href =
					componenthome +
					"/results/changeresultview.html?oemaxlevel=1&cache=false&hitsperpage=" +
					originalhitsperpage;
			} else {
				var href =
					componenthome + "/results/changeresultview.html?oemaxlevel=1";
			}
			var args = {
	...
	lQuery(".resultsselection input.selectionbox").livequery(
		"change",
		function (e) {
			var clicked = $(this);
...
checkScroll = function () {
	...
	var atbottom =
		$(window).scrollTop() + (visibleHeight * 2 + 500) >= totalHeight; //is the scrolltop plus the visible equal to the total height?
	if (!atbottom) {
