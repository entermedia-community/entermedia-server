	...
	jQuery.fn.html5_upload = function (options) {
		...
		var options = jQuery.extend(
			{
				...
				headers: {
					...
					"X-File-Name": function (file) {
						return encodeURI(get_file_name(file));
					},
		...
		function upload() {
			...
			function upload_file(number) {
				...
				if (
					!$this.triggerHandler("html5_upload.onStartOne", [
						get_file_name(file),
						number,
						total,
					])
				...
				xhr.upload["onprogress"] = function (rpe) {
					$this.triggerHandler("html5_upload.onProgress", [
				...
				xhr.onload = function (load) {
					if (xhr.status != 200) {
						...
						$this.triggerHandler("html5_upload.onFinishOne", [
							xhr.responseText,
							get_file_name(file),
							number,
							total,
						...
						options.setStatus(options.genStatus(1, true));
						options.setProgress(
							options.genProgress(get_file_size(file), get_file_size(file))
						);
						upload_file(number + 1);
					}
				};
				...
				xhr.open(
					options.method,
					typeof options.url == "function" ? options.url(number) : options.url,
					true
				);
